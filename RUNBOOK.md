...
| `CLOUD_EVIDENCE_INVENTORY_WORKBOOK` | `0` | Set to `1` (or pass `--inventory-workbook`) to enumerate cloud assets and emit the FedRAMP Appendix M Integrated Inventory Workbook (`out/inventory-workbook.{csv,xlsx}`) for AWS + GCP. Read-only; needs the asset-enumeration permissions in `IAM-PERMISSIONS-CATALOG.md` (`inventory-assets.ts`). |
