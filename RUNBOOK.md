...
| `CLOUD_EVIDENCE_OSCAL_SSP` | `0` | Set to `1` (or pass `--oscal-ssp`) to emit a **draft** OSCAL 1.1.2 System Security Plan (`out/ssp.json`) bootstrapped from the run's evidence: one `implemented-requirement` per FedRAMP Rev5 baseline control (for the run's impact level), with implementation-status derived from the control benchmark (satisfied→implemented, mixed→partial, failed/unassessed→planned) and a narrative citing the KSIs/rules. Validated against the committed NIST SSP schema; covered by the signed manifest. A starting point for the system owner — not a final SSP. |
| `CLOUD_EVIDENCE_SSP_DOCX` | `0` | Set to `1` (or pass `--ssp-docx`) to also render the SSP to a FedRAMP-style Word document (`out/ssp.docx`). Implies `--oscal-ssp`. Dependency-free OOXML (store-only ZIP). The `.docx` is not in the signed manifest (the signer covers `*.json`), but it's a faithful render of the signed `ssp.json` — reproducible from the signed source. |
| `CLOUD_EVIDENCE_SYSTEM_NAME` | `Cloud System` | System name embedded in the OSCAL SSP (`--system-name` overrides). |
| `CLOUD_EVIDENCE_SYSTEM_ID` | `cloud-evidence-system` | System identifier embedded in the OSCAL SSP (`--system-id` overrides). |
| `CLOUD_EVIDENCE_SYSTEM_DESCRIPTION` | (auto) | System description embedded in the OSCAL SSP. |
| `CLOUD_EVIDENCE_INVENTORY_WORKBOOK` | `0` | Set to `1` (or pass `--inventory-workbook`) to enumerate cloud assets (generic discovery backbone + per-service depth, all regions) and emit `out/inventory.json` (rich superset), `inventory-workbook.{csv,xlsx}` (FedRAMP Appendix M), `inventory-oscal.json`, `inventory-cmdb.json`, `inventory-diff.json`, `inventory-cost.json` for AWS + GCP. Read-only; permissions per `IAM-PERMISSIONS-CATALOG.md` (`inventory-assets.ts`, `discover.ts`, `inventory-cost.ts`). Use `--inventory-only` for a fast inventory-focused run that skips KSI/process evidence. |
| `CLOUD_EVIDENCE_SCN` | `0` | Set to `1` (or pass `--scn`) to classify the run's diff (findings + inventory + optional proposed changes) as a FedRAMP **Significant Change Notification** report. Emits `out/scn-classification.json` and `out/scn-notice-draft.md` (a starting-point notice email for the authorizing agency). Implies `--diff-report` so the input is produced. Clean-room from the huntridge-labs/argus AGPL idea. |
| `CLOUD_EVIDENCE_SCN_PROPOSED_PATH` | (none) | Path to a JSON file of operator-proposed changes (forward-looking) to include in the SCN classification — either an array of `ScnChange` records or `{changes:[...]}`. |
| `CLOUD_EVIDENCE_REFERENCE_ARCH` | `0` | Set to `1` (or pass `--reference-arch`) to audit the running environment against FedRAMP reference-architecture hardening (Coalfire RAMPpak-derived, clean-room) and emit `out/AUDIT-REFARCH-AWS.json` (10 checks) + `out/AUDIT-REFARCH-GCP.json` (13 checks). Read-only; the findings flow into `control-benchmark.json`, the family roll-up (`REFARCH`), the crosswalk, OSCAL, and the signed manifest. These are hardening *audits*, not KSI obligations, so they are excluded from the KSI pass/fail rollup. Permissions per `IAM-PERMISSIONS-CATALOG.md` (`reference-arch.ts`). |
| `CLOUD_EVIDENCE_APPROVED_AMI_PATTERN` | (none) | Regex of approved/STIG-hardened AMI IDs. When set, `--reference-arch` flags running EC2 instances launched from an AMI outside the pattern (`aws.ec2.approved_ami_provenance`). Informational until set. |
| `CLOUD_EVIDENCE_GCP_API_ALLOWLIST` | (none) | Comma-separated list of expected enabled GCP service APIs (e.g. `compute.googleapis.com,storage.googleapis.com`). When set, `--reference-arch` flags any enabled API outside the allow-list (`gcp.serviceusage.curated_apis`). Informational until set. |
