...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs in `providers/azure/iam.ts`). **MLA + CMT-LMC** (5 KSIs in `providers/azure/logging.ts`). **CNA + SVC-SNT** (5 KSIs in `providers/azure/network.ts`). **Config / Policy** (2 KSIs in `providers/azure/config.ts`): CNA-EIS + CNA-IBP — Azure Policy assignments + state evaluations + Microsoft Cloud Security Benchmark + FedRAMP/NIST regulatory initiative detection. **19 KSIs Azure-covered**, 93 dedicated tests + smoke. **Backlog:** CNA-DFP / CNA-OFA, MLA-EVC, INR-RIR, then backup, supplychain, secrets, data, inventory. |
