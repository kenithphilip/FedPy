...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (`providers/azure/iam.ts`): MFA / ELP / AAM / APM / SNU / JIT / SUS — all 7 IAM KSIs are now AWS + GCP + Azure via Microsoft Graph (Conditional Access + Security Defaults + PIM + dormancy + authenticationStrength + SP credential hygiene + JIT activations + Identity Protection risk-based CA). 40 dedicated tests + smoke. **Backlog:** network, config, backup, logging, supplychain, secrets, data, inventory — to be split per family in follow-up commits. |
