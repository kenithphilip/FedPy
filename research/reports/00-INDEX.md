...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs in `providers/azure/iam.ts`). **MLA + CMT-LMC** (`providers/azure/logging.ts`): MLA-LET + MLA-OSM + MLA-ALA + MLA-RVL + CMT-LMC. **CNA + SVC-SNT** (`providers/azure/network.ts`): CNA-MAT + CNA-RNT + CNA-ULN + CNA-RVP + SVC-SNT — attack-surface, traffic restriction, NSG flow logs, WAF, TLS-only. **17 KSIs Azure-covered**, 84 dedicated tests + smoke. **Backlog:** CNA-EIS / CNA-IBP / CNA-DFP / CNA-OFA, MLA-EVC, INR-RIR, then config, backup, supplychain, secrets, data, inventory. |
