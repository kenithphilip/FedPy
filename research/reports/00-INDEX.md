...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (`providers/azure/iam.ts`): all 7 IAM KSIs (MFA / ELP / AAM / APM / SNU / JIT / SUS). **MLA started** (`providers/azure/logging.ts`): MLA-LET + MLA-OSM via Resource Graph (diagnostic settings + Log Analytics + Sentinel onboarding). 50 dedicated tests + smoke. **Backlog:** MLA-ALA + MLA-EVC + MLA-RVL + CMT-LMC + INR-RIR (rest of logging family), then network, config, backup, supplychain, secrets, data, inventory. |
