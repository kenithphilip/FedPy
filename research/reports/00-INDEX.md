...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **Done:** AZ-IAM-MFA (`providers/azure/iam.ts::collectIamMfa`) — Conditional Access + Security Defaults via Microsoft Graph; KSI-IAM-MFA now AWS + GCP + Azure; established the per-KSI Azure dispatch + Graph access pattern future Azure collectors reuse (9 dedicated tests + smoke). **Backlog:** IAM-ELP, IAM-AAM, IAM-SUS, IAM-APM, IAM-SNU, IAM-JIT, network, config, backup, logging, supplychain, secrets, data, inventory — to be split per family in follow-up commits. |
