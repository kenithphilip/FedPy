...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs). **MLA + CMT-LMC** (6 KSIs in `logging.ts` — LET / OSM / ALA / RVL / CMT-LMC / EVC). **CNA + SVC-SNT** (5 KSIs in `network.ts`). **Config / Policy** (3 KSIs in `config.ts` — EIS / IBP / DFP). **Backup / Availability + Recovery** (3 KSIs in `backup.ts` — CNA-OFA / RPL-ABO / RPL-TRC). **24 KSIs Azure-covered**, 115 dedicated tests + smoke. **Backlog:** INR-RIR, RPL-ARP / RRO (hybrids), supplychain, secrets, data, inventory. |
