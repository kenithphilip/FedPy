...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs in `providers/azure/iam.ts`). **MLA + CMT-LMC done** (`providers/azure/logging.ts`): MLA-LET + MLA-OSM + MLA-ALA + MLA-RVL + CMT-LMC — workspace RBAC, retention, alert rules, Sentinel onboarding, activity-log export, change-tracking. 63 dedicated tests + smoke. 12 of ~13 KSI families addressed (Azure-side of KSI-IAM is 7/7, MLA is 4/5, CMT is 1/4). **Backlog:** MLA-EVC, INR-RIR (rest of logging-adjacent), then network, config, backup, supplychain, secrets, data, inventory. |
