...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **Done:** AZ-IAM-MFA / AZ-IAM-ELP / AZ-IAM-AAM (`providers/azure/iam.ts`) — Conditional Access + Security Defaults + Global-Admin/PIM + dormancy via Microsoft Graph; KSI-IAM-MFA / KSI-IAM-ELP / KSI-IAM-AAM are AWS + GCP + Azure (established the per-KSI Azure dispatch + Graph access pattern; 22 dedicated tests + smoke). **Backlog:** IAM-SUS, IAM-APM, IAM-SNU, IAM-JIT, network, config, backup, logging, supplychain, secrets, data, inventory — to be split per family in follow-up commits. |
