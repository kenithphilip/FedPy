...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs). **MLA + CMT-LMC + INR-RIR** (7 KSIs in `logging.ts` — LET / OSM / ALA / RVL / CMT-LMC / EVC / INR-RIR). **CNA + SVC-SNT** (5 KSIs in `network.ts`). **Config / Policy** (3 KSIs in `config.ts` — EIS / IBP / DFP). **Backup / Availability + Recovery** (3 KSIs in `backup.ts` — CNA-OFA / RPL-ABO / RPL-TRC). **Secrets / Key Vault** (1 KSI in `secrets.ts` — SVC-ASM). **26 KSIs Azure-covered**, 127 dedicated tests + smoke. **Backlog:** RPL-ARP / RRO (hybrids), supplychain (CMT-RMV / CMT-VTD / SCR-MON), data (SVC-RUD / SVC-VCM / SVC-VRI), config (SVC-ACM / SVC-EIS), inventory (PIY-GIV). |
