...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs in `providers/azure/iam.ts`). **MLA + CMT-LMC** (`providers/azure/logging.ts`): MLA-LET + MLA-OSM + MLA-ALA + MLA-RVL + CMT-LMC. **CNA + SVC-SNT started** (`providers/azure/network.ts`): CNA-ULN + CNA-RVP + SVC-SNT — NSG flow logs, WAF, TLS-only AGW listeners + storage HTTPS-only. **15 KSIs Azure-covered**, 75 dedicated tests + smoke. **Backlog:** rest of CNA (MAT, RNT, EIS, IBP, DFP, OFA), MLA-EVC, INR-RIR, then config, backup, supplychain, secrets, data, inventory. |
