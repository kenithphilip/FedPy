...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs). **MLA + CMT-LMC** (6 KSIs in `logging.ts` — LET / OSM / ALA / RVL / CMT-LMC / EVC). **CNA + SVC-SNT** (5 KSIs in `network.ts`). **Config / Policy** (3 KSIs in `config.ts` — EIS / IBP / DFP). **Backup / Availability** (1 KSI in new `backup.ts` — CNA-OFA). **22 KSIs Azure-covered**, 104 dedicated tests + smoke. **Backlog:** INR-RIR, then RPL family (ABO / TRC / ARP / RRO), supplychain, secrets, data, inventory. |
