...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs). **MLA + CMT-LMC + INR-RIR** (7 KSIs in `logging.ts` — LET / OSM / ALA / RVL / CMT-LMC / EVC / INR-RIR). **CNA + SVC-SNT** (5 KSIs in `network.ts`). **Config / Policy / Posture** (5 KSIs in `config.ts` — CNA-EIS / CNA-IBP / CNA-DFP / SVC-ACM / SVC-EIS). **Backup / Availability + Recovery** (3 KSIs in `backup.ts` — CNA-OFA / RPL-ABO / RPL-TRC). **Secrets / Key Vault** (1 KSI in `secrets.ts` — SVC-ASM). **Supply chain** (2 KSIs in `supplychain.ts` — CMT-RMV / CMT-VTD). **Data plane** (3 KSIs in `data.ts` — SVC-RUD / SVC-VCM / SVC-VRI). **33 KSIs Azure-covered**, 164 dedicated tests + smoke. **Backlog:** RPL-ARP / RRO (hybrids), SCR-MON (supply-chain monitoring), inventory (PIY-GIV). |
