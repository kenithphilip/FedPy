...
| OSC-3 | ✅ done | OSCAL XML output via pure-JS converter (`core/oscal-xml.ts`): deterministic JSON→XML per the OSCAL metaschema mapping (flag-keys → XML attributes, plural keys → repeated singular elements, prose fields → `<p>` wrapping). Both emitters (`assessment-results` + `ssp`) now drop sibling `.xml` files by default; `CLOUD_EVIDENCE_DISABLE_OSCAL_XML=1` opts out. The `.xml` payload is included in the signed run manifest alongside `.json` and the ephemeral signing `.pem` pair, so downstream FedRAMP tooling (oscalkit / GoComply/fedramp) gets an integrity-verified XML artifact without shelling out to NIST XSLT or Saxon (offline-first invariant preserved). 14 dedicated tests. |
...
| AZ-CHK | ✅ done | Azure reference-architecture audit (`providers/azure/reference-arch.ts`, `--reference-arch`): 11 KQL checks (Defender, FedRAMP policy, storage public-blob/HTTPS/network, Key Vault hardening, CMK, disk encryption, NSG open admin, no VM public IPs, workspace retention); feeds benchmark/family-rollup/crosswalk/OSCAL but excluded from KSI pass-fail (5 tests) |
| AZ-2 | ✅ complete | Azure per-KSI collectors at full parity with AWS + GCP. **IAM family** (7 KSIs). **MLA + CMT-LMC + INR-RIR** (7 KSIs in `logging.ts`). **CNA + SVC-SNT** (5 KSIs in `network.ts`). **Config / Policy / Posture** (5 KSIs in `config.ts`). **Backup / Availability + Recovery** (5 KSIs in `backup.ts` — CNA-OFA / RPL-ABO / RPL-TRC / RPL-ARP / RPL-RRO). **Secrets / Key Vault** (1 KSI in `secrets.ts`). **Supply chain** (3 KSIs in `supplychain.ts`). **Data plane** (3 KSIs in `data.ts`). **Inventory** (1 KSI in `inventory.ts`). **HYBRID closeouts** (7 KSIs in `ksi-hybrids.ts` + `crypto.ts` + `vdr-scan.ts` — CMT-RVP / INR-AAR / INR-RPI / SCR-MIT / SVC-PRR / AFR-UCM / AFR-VDR). **All 44 collector-tracked KSIs at AWS=GCP=Azure parity. 209 dedicated Azure tests + smoke, 733 tests total.** |
| SCN-1 | ✅ done | Significant Change Notification classifier (`core/scn-classifier.ts`, `--scn`): rule library covering FedRAMP significant-change taxonomy (boundary / auth / crypto / network / data-flow / personnel / platform-version / subprocessor / config / improvement), harvest from diff-report.json + inventory-diff.json + optional proposed-changes JSON, emits `scn-classification.json` + `scn-notice-draft.md` starting-point notice (18 tests). Clean-room from argus AGPL idea. |
