...
| AZ-2 | 🟨 in-progress | Azure per-KSI collectors. **IAM family complete** (7 KSIs). **MLA + CMT-LMC + INR-RIR** (7 KSIs in `logging.ts`). **CNA + SVC-SNT** (5 KSIs in `network.ts`). **Config / Policy / Posture** (5 KSIs in `config.ts`). **Backup / Availability + Recovery** (3 KSIs in `backup.ts`). **Secrets / Key Vault** (1 KSI in `secrets.ts`). **Supply chain** (3 KSIs in `supplychain.ts` — CMT-RMV / CMT-VTD / SCR-MON). **Data plane** (3 KSIs in `data.ts`). **Inventory** (1 KSI in `inventory.ts` — PIY-GIV). **35 KSIs Azure-covered**, 174 dedicated tests + smoke. **Backlog:** RPL-ARP / RPL-RRO (HYBRID stubs, will mirror the AWS/GCP `ksi-hybrids.ts` pattern). |
