...
### Added — LOOP-C.C9: Baseline Configuration document `.docx` emitter (CM-2) — COMPLETES LOOP-C

Shipped `core/baseline-config-emit.ts` — the **NIST SP 800-53 Rev. 5 CM-2 (Baseline Configuration)** document (`out/baseline-config.docx`): the CSP's documented, approved baseline of record. Rev. 5 split CM-2 (the approved configuration each component is authorized to run) from CM-8 (the inventory of what components exist); this is the CM-2 half, and it is DISTINCT from both the CM-8 Integrated Inventory Workbook and the AFR Secure Configuration Guide (LOOP-G.G5, the *recommended* secure configuration). It is the baseline the Configuration Management Plan (LOOP-C.C1, CMP §5) cross-links to and the basis LOOP-E's continuous monitoring uses to detect drift. **CM-2(2) (Automation Support for Accuracy and Currency)** is satisfied by auto-deriving the baseline from real evidence each run; **CM-2(7) (High-Risk Areas)** is addressed via the §4 hardening expectations. §1 quotes **CM-2(a)** and **NIST SP 800-128 §3.2** (Guide for Security-Focused Configuration Management) verbatim; §4 cites the **CIS Benchmarks** (referenced, not copied — CIS content is licensed). The emitter is a pure builder (`buildBaselineConfigBodyXml` / `renderBaselineConfigDocx`) + disk emitter (`emitBaselineConfigDocx`) with exported readers `readInventoryBaselineRows` / `readReferenceArchitecture(sAllProviders)` / `diffInventoryVsReference` / `buildBaselineItemRows` / `componentClassFor`.

**Real evidence path (REO).** The 8-section body auto-derives from three real sources, each cited by content SHA-256 in the provenance footer. **§3 Baseline Configuration Items** groups the run's real `out/inventory.json` (CM-8) by (cloud provider, normalized component class — compute/container/database/storage/network/identity/serverless), so ephemeral / auto-scaling assets collapse into their class baseline instead of fabricating a per-instance one; each row shows the current image(s) + documented hardening baseline. **§4 Reference Architecture** is grep-read from the `providers/{aws,gcp,azure}/reference-arch.ts` SOURCE — each documented hardening expectation (`finding({ rule, ... severity, ... target:{summary}, ... nist_controls:[...] })`) is extracted directly, the same grep-against-source idiom `roe-emit.ts` uses on `ksi-map.ts`, so no provider cloud SDK client loads at emit time (34 expectations parsed this build: AWS 10 / GCP 13 / Azure 11). **§5 Deviations from Baseline** is a pure diff of the real inventory rows against the §4 hardening anchors, computing two honest, source-grounded deviation kinds — undocumented-hardening-baseline (a compute/container asset with an empty `baselineConfig` where the provider documents an approved-image anchor) and encryption-at-rest-disabled (where the provider documents an encryption anchor) — with each deviation's severity taken from the anchoring reference finding's DECLARED severity, never defaulted (Risk 4). Absent inputs degrade to `REQUIRES-OPERATOR-INPUT`: no `inventory.json` → §3/§5 degrade; no readable `reference-arch.ts` → §4/§5 degrade (never crashes the dispatch — REO Rule 4). Operator identity fields — baseline approver (§8), deviation-log location (§7), review cadence (§6, default `annually` per CM-2) — come from `config.yaml: baseline_config.*`; the §8 approval signature is NEVER auto-signed. The document is fully deterministic (no wall-clock time): the metadata UUID is `deterministicUuid('baseline-config:' + systemId + ':' + runId)` and the provenance footer cites the SHA-256 of `inventory.json` + each `reference-arch.ts`, so identical inputs yield a byte-identical `.docx`. Integrity is anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519), the same coverage the eight sibling LOOP-C `.docx` receive (`.docx` is intentionally not in `sign.ts` SIGNED_EXTENSIONS — the bundle records + signs its digest).

**Orchestrator wiring.** New `--baseline-config` flag (env `CLOUD_EVIDENCE_BASELINE_CONFIG`) + a `config.yaml: baseline_config` section (`approver` / `deviation_log` / `review_cadence` / `configuration_items_override`). The dispatch runs BEFORE `--cmp` so the CMP §5 baseline cross-link (`--cmp-baseline-config-href`) auto-resolves to `baseline-config.docx` when co-emitted (an `existsSync` check on the outDir), and BEFORE signing so the `.docx` is bundle-covered. One `submission-bundle` WELL_KNOWN role registered (`baseline-config-docx` → `baseline-config.docx`). **Shipping C.C9 COMPLETES LOOP-C (Document Template Pack), 9 of 9 slices — the 5th core loop complete (after A, W, T, B).**

**Verification.** `npm run typecheck` clean; **1601 tests passing (+15 for C.C9: 13 §8 contract tests + a provenance-SHA-256 test + a `componentClassFor` normalization unit)** — the only 2 non-passing in the full parallel run are the pre-existing load-flaky `orchestrator-phase-f` subprocess tests (one hit the 10s spawn timeout under full-suite load; both pass 5/5 in isolation and never touch the `baselineConfig` branch). `npm run check:reo` green (G1 lint:no-stubs 0 violations / 191 files; G3 check:provenance OK; check:ssdf-no-silent-pass OK; G2 coverage-regression SKIP — no collector run in this env). Files: `core/baseline-config-emit.ts` (+903), `tests/core/baseline-config-emit.test.ts` (+331), 2 fixtures (`inventory.sample.json` +94 / `config.sample.yaml` +17); modified `core/submission-bundle.ts` (+2 — role union + WELL_KNOWN), `core/orchestrator.ts` (+101 — flag/env/`Config.baseline_config`/dispatch/help + CMP href auto-resolve). Spec reconciliations recorded in LOOP-C-RISKS C-C9-8..11 (`reference-arch.ts` are live SDK collectors not static arrays → grep the finding blocks; `scg-comparator.ts` compares a settings-map not inventory rows → local pure `diffInventoryVsReference`; DEPENDENCY-GRAPH lists C.C9 ← D.D1–D.D3 but the per-slice-doc contract does not, so C.C9 shipped ahead of LOOP-D per §4-is-source-not-diagram; docx-primitives still not extracted — 15th emitter for C-X-1).

### Added — LOOP-C.C8: Authorization request cover letter `.docx` emitter (PM-10)

Shipped `core/auth-cover-letter-emit.ts` — the CSP-side authorization-request cover letter / package transmittal (`out/auth-request-cover-letter.docx`) that ACCOMPANIES a FedRAMP authorization package when it is transmitted to the FedRAMP PMO or an agency Authorizing Official. It satisfies **NIST SP 800-53 Rev. 5 PM-10 (Authorization Process)** and is distinct from the AO-issued ATO letter (the AO's RESPONSE to this cover letter). The 7-section body (Letterhead → Addressee → §1 Subject → §2 Request Summary → §3 Independent Assessment (3PAO) Statement → §4 Package Contents → §5 Requested Action → §6 Primary Contacts → §7 Closing + Signature) follows the **FedRAMP Agency Authorization Playbook v4.1 (2025-11-17) §3** formal-request structure; §1 quotes PM-10(a) verbatim and cites **NIST SP 800-37 Rev. 2 §3.6 (Authorize)**; §4 mirrors the **FedRAMP Initial Authorization Package Checklist**; §5 references the **FedRAMP ATO Letter Template** as the expected AO response. The emitter is a pure builder (`buildCoverLetterBodyXml` / `renderAuthCoverLetterDocx`) + disk emitter (`emitAuthCoverLetterDocx`) with exported readers `readIndexJson` / `readApMetadata` and resolvers `resolveThirdParty` / `resolveSubmissionDate`.

**Real evidence path (REO).** §4 Package Contents auto-enumerates the run's signed `out/INDEX.json` (LOOP-A.A4 submission-bundle) — one row per artifact with filename + role + the REAL sha256-short (first 12 hex chars of the per-artifact digest) + bytes, alphabetically sorted for stable diffing — plus the true artifact count and the INDEX.json SHA-256. §3 3PAO Statement reads `out/ap.json` (LOOP-A.A2 OSCAL Assessment Plan) metadata for the assessing 3PAO organization and the AP finalization date. Each absent LOOP-A input degrades to a `REQUIRES-OPERATOR-INPUT` marker with a "run with --submission-bundle / --oscal-ap" cross-link (never blocks — REO Rule 4). Operator identity fields — CSP executive signatory (§7), CSP mailing address (letterhead), AO addressee (with optional delegated-designee), and 3PAO lead assessor — are operator-supplied via `config.yaml: auth_request.*` + `config.yaml: org.address`; the executive signature block is NEVER auto-signed (the `.docx` is Ed25519-signed by the pipeline; the wet/electronic executive signature is separate). The document is fully deterministic (no wall-clock time): the metadata UUID is `deterministicUuid('auth-cover-letter:' + systemId + ':' + runId)` and the date line resolves `submissionDate` → an ISO date embedded in the runId → a `REQUIRES-OPERATOR-INPUT` "dated at signing" marker, so identical inputs yield a byte-identical `.docx`.

**Orchestrator wiring.** New `--auth-cover-letter` (env `CLOUD_EVIDENCE_AUTH_COVER_LETTER`) + `--auth-request-type initial-ato|continued-ato|reauthorization` flags; new `config.yaml` `org` + `auth_request` sections. Because the shipped `emitSubmissionBundle` is monolithic (INDEX.json + tarball in one atomic call), when co-run with `--submission-bundle` the orchestrator runs a pre-pass bundle (`strict:false`) to materialize INDEX.json, emits the letter reading it, then the normal bundle dispatch re-packs the tarball so the letter is enclosed and enumerated in the final INDEX.json — realizing the spec's INDEX-build → cover-letter → bundle-pack order. One `submission-bundle` WELL_KNOWN role registered (`auth-cover-letter-docx`, conditional).

**Verification.** `npm run typecheck` clean; **1586/1586 tests passing** (+18 for C.C8: 11 §8 contract tests + 7 extras — 3PAO resolution precedence, deterministic date fallback, real sha256-short + alphabetical sort, INDEX.json reader tolerance of extra fields, §3 degrade path, config.sample.yaml round-trip, structured log event); `npm run check:reo` green (G1 lint:no-stubs 0 violations; G3 check:provenance OK; G2 coverage-regression SKIP — no collector run in this env; check:ssdf-no-silent-pass OK). Smoke run: valid `.docx` (`unzip -t`, 6 OOXML parts), 12 package artifacts enumerated, 3PAO resolved from ap.json, `ready_for_signature`. Files: `core/auth-cover-letter-emit.ts` (+749), `tests/core/auth-cover-letter-emit.test.ts` (+336), 3 fixtures (INDEX.sample.json / ap.sample.json / config.sample.yaml); modified `core/submission-bundle.ts` (+2), `core/orchestrator.ts` (+136). Spec reconciliations recorded in LOOP-C-RISKS C-C8-7..10 (INDEX.json `built_at`/`run_id` shape; ap.json parties carry no per-party role; monolithic bundler → two-pass dispatch; docx-primitives still not extracted — 14th emitter for C-X-1).

### Added — LOOP-C.C7: Risk Management Strategy `.docx` emitter (PM-9)

Shipped the Risk Management Strategy — the signed-by-bundle `out/rms.docx` (PM-9), an 11-section Word document that frames how the CSP manages risk end to end (framing → assessing → responding → monitoring, per NIST SP 800-39 §2) and sits ABOVE the per-system SSP and the per-finding POA&M in the NIST SP 800-39 §2.1 three-tier risk hierarchy (Organization → Mission/Business Process → Information System). New `core/rms-emit.ts` (pure `buildRmsBodyXml` / `renderRmsDocx` + disk `emitRmsDocx`, with exported readers `readRiskRegister` / `readAcceptancePolicy` / `summarizePoam` and the typed `PoamSeverityError`) auto-links the risk-posture sections to the run's REAL risk corpus: **§5 Risk Register Reference** reads `out/risk-register.json` (LOOP-B.B5, RA-3) and embeds its top-line summary (entries_total / open_count / high_inherent_count / by_source + evidence SHA-256 — Open Q1: a summary, not a full row dump); **§6 Risk Acceptance Policy** summarizes the `out/.risk-acceptances.json` (LOOP-B.B3) + `out/.compensating-controls.json` (LOOP-B.B4) signed snapshots (counts + SHA-256s); **§10 POA&M Summary** parses `out/poam.json` (LOOP-A.A1) into a severity histogram over poam-items, a count of overdue open items (deadline < now, status ≠ closed), and the oldest-open-finding age (from the earliest related observation `collected` to now — Open Q4: currently-open only). Each absent LOOP-A/B input degrades to a `REQUIRES-OPERATOR-INPUT` marker with a "Generate via LOOP-B / --oscal-poam" cross-link (Open Q2: degrade, never block — REO Rule 4). **§4 Risk Response Strategy** renders the NIST SP 800-39 Accept / Avoid / Mitigate / Transfer 4-row matrix (published terminology, REO Rule 3; Open Q3: FedRAMP is silent on a Moderate-vs-High preference, so the generic matrix applies). **§7 Continuous Risk Monitoring** cross-links to the C.C6 ConMon Strategy when `out/conmon-strategy.docx` is present, else a `REQUIRES-OPERATOR-INPUT` marker. **§8 Risk Tolerance** + **§9 Executive Oversight** are operator-supplied. Extended `core/submission-bundle.ts` (WELL_KNOWN role `rms-docx`), `core/orchestrator.ts` (`--rms` flag + `CLOUD_EVIDENCE_RMS` env + a `config.yaml#rms.*` section, dispatched after the ConMon Strategy emit and before signing, with a console warning when the LOOP-B register is absent), `config.yaml` (new `rms:` section), and `docs/OPERATOR-GUIDE.md` (§3 flag / §4 env / §7 output).

Verification: typecheck clean; the cloud-evidence suite rose 1514 → 1533 (+19: `tests/core/rms-emit.test.ts` — the 12 per-slice §8 tests plus 6 extras [the §10 overdue + oldest-open-age computation, the `PoamSeverityError` guard, the §6 acceptance-policy summary, the §7 ConMon cross-link, the `config.sample.yaml` fixture round-trip, and the structured log event] and 1 typed-error case), 100% passing across 119 files; `npm run check:reo` returns 0 (lint:no-stubs 0 violations, check:provenance OK, coverage-regression SKIP with no local `out/inventory-coverage.json`, ssdf-no-silent-pass OK). A smoke run emitted a valid `rms.docx` (6 OOXML parts, `unzip -t` clean; register linked, 5 POA&M items, ready_for_signature) and re-emitting over identical inputs produced a byte-identical file.

Statutory / regulatory drivers (verbatim from the per-slice §2): NIST SP 800-53 Rev. 5 PM-9 ("Develops a comprehensive strategy to manage: 1. Security risk to organizational operations and assets, individuals, other organizations, and the Nation associated with the operation and use of organizational systems; and 2. Privacy risk to individuals resulting from the authorized processing of personally identifiable information; b. Implements the risk management strategy consistently across the organization; and c. Reviews and updates the risk management strategy [Assignment: organization-defined frequency] or as required, to address organizational changes."), with PM-8 / RA-1 / RA-3 as supporting controls; NIST SP 800-39 (2011-03) §2 ("The risk management process involves four components: (i) framing risk; (ii) assessing risk; (iii) responding to risk; (iv) monitoring risk.") quoted verbatim in §1 + the §2.1 three-tier hierarchy; NIST SP 800-37 Rev. 2 (2018-12) §3 RMF steps cited in §1; NIST SP 800-30 Rev. 1 (2012-09) §3.2 assessment methodology cited in §3.

REO compliance: §5 / §6 / §10 trace to real on-disk LOOP-A/B files (never a fabricated register, acceptance count, or POA&M summary); an unknown POA&M severity throws the typed `PoamSeverityError` (no silent mis-bucket); the document is fully deterministic (UUID = `deterministicUuid('rms:'+systemId+':'+runId)`; the §10 "now" for the overdue + oldest-open-age computation derives from the POA&M `metadata.last-modified` — an input datum, injectable via `opts.now` for tests — never a wall-clock `Date.now()`) so identical inputs produce a byte-identical `.docx`, its integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519); executive-oversight names (§9) are operator-supplied via `config.yaml: rms.executive_oversight[]` and never logged (only counts); `ready_for_signature` gates on tolerance + executive + register link + acceptance policy + system identity. Open questions Q1–Q5 all resolved (see the C.C7 scope note in `docs/STATUS.md`). Spec reconciliation: the real POA&M severity enum is `critical/high/medium/low/info` (`core/envelope.ts` — the fourth bucket is `medium`, not the per-slice §schema's `moderate`) and the B.B3/B.B4 snapshots are the signed dotfiles `.risk-acceptances.json` / `.compensating-controls.json` (each an object with an `items[]` array) — `summarizePoam` buckets by the honest enum and `readAcceptancePolicy` reads the dotfile first with the non-dotted form as a fallback (LOOP-C-RISKS C-C7-7); the shared `core/docx-primitives.ts` remains unextracted — 13 emitters now carry local OOXML constants (C-C7-8 / C-X-1).

### Added — LOOP-C.C6: Continuous Monitoring Strategy + Plan `.docx` emitter (CA-7 / CA-7(1) / PM-31)

Shipped the Continuous Monitoring Strategy + Plan — the signed-by-bundle `out/conmon-strategy.docx` (CA-7 / CA-7(1) / PM-31), a 13-section Word document that names WHICH controls are under continuous monitoring, at WHAT frequency, backed by WHAT scanning evidence, with WHAT escalation, reported to WHICH FedRAMP endpoint. LOOP-E (the ConMon agent) reads this document as its configuration for the monthly ConMon runs. New `core/conmon-strategy-emit.ts` (pure `readKsiCatalog` / `readVdrScanners` / `buildConmonStrategyBodyXml` / `renderConmonStrategyDocx` + disk `emitConmonStrategyDocx`) auto-derives **§4 Controls Under Continuous Monitoring** from the live `core/ksi-map.ts` — `readKsiCatalog()` greps the source for each KSI id + its real `scope` (the same trick as `roe-emit.ts:readKsiScope`, so no provider collector modules are pulled into the emit bundle) and derives the Automated column from the scope (CLOUD → automated cloud-SDK evidence, HYBRID → automated signal + process artifact, PROCESS → tracker artifact, INHERITED → leveraged authorization); `buildConmonStrategyBodyXml` throws `ConmonKsiScopeError` if fewer than 20 KSIs resolve (the live map has 44). **§5 Vulnerability Scanning** auto-derives from the run's real `KSI-*VDR*` evidence via `readVdrScanners()` — one row per provider block (= one scanner), the scanner name read from the collector's own `<provider>.vdr.detection_capability_enabled` finding, detection-enabled from `finding.passed`, findings/KEV counts from the `vdr.summary` evidence, last-collected from the envelope `collected_at`, plus the evidence-file SHA-256. Extended `core/submission-bundle.ts` (WELL_KNOWN role `conmon-strategy-docx`), `core/orchestrator.ts` (`--conmon-strategy` flag + `CLOUD_EVIDENCE_CONMON_STRATEGY` env + a `config.yaml#conmon.*` section, dispatched after the FIPS 199 emit and before signing), `config.yaml` (new `conmon:` section), and `docs/OPERATOR-GUIDE.md` (§3 flag / §4 env / §7 output).

Verification: typecheck clean; the cloud-evidence suite rose 1495 → 1514 (+19: `tests/core/conmon-strategy-emit.test.ts` — the 13 per-slice §8 tests plus 6 extras: reporting-endpoint override, disabled-scanner row, `.signed.json`/`.json` dedupe, `ConmonKsiScopeError`, the `config.sample.yaml` fixture round-trip, and the structured log event), 100% passing across 118 files; `npm run check:reo` returns 0 (lint:no-stubs 0 violations, check:provenance OK, coverage-regression SKIP with no local `out/inventory-coverage.json`, ssdf-no-silent-pass OK). A smoke run emitted a valid `conmon-strategy.docx` (6 OOXML parts, `unzip -t` clean; 44 KSIs in §4, 2 scanners in §5, endpoint usda-connect.gov).

Statutory / regulatory drivers (verbatim from the per-slice §2): NIST SP 800-53 Rev. 5 CA-7 ("Develop a system-level continuous monitoring strategy and implement continuous monitoring in accordance with the organization-level continuous monitoring strategy that includes: a. Establishing … system-level metrics …; b. Establishing … frequencies … for monitoring and … for assessment of control effectiveness; …; g. Reporting the security and privacy status of the system to [personnel/roles] [frequency]."), CA-7(1) (Independent Assessment), PM-31 (Continuous Monitoring Strategy); FedRAMP Continuous Monitoring Strategy Guide v3.2 (2018-04-04) §3.1 quoted verbatim in §3 ("The CSP is required to perform continuous monitoring of all security controls in the SSP at the frequency identified by the FedRAMP requirements and as stated in the [CSP] System Security Plan."); FedRAMP Continuous Monitoring Playbook v1.0 (2025-11-17) cited as the cadence source; NIST SP 800-137 §2.1 ("Information security continuous monitoring (ISCM) is defined as maintaining ongoing awareness of information security, vulnerabilities, and threats to support organizational risk management decisions.") + §3 three-tier hierarchy; NIST SP 800-137A assessment-method classifications; RFC-0026 (Clarifying CA-7 Continuous Monitoring Expectations for Rev5 Providers); CISA BOD 22-01 (KEV catalog) for the §11 escalation baseline; the R2 research finding (monthly full-document POA&M re-upload to USDA Connect.gov for Low/Moderate) for §9.

REO compliance: §4 traces to the live `core/ksi-map.ts` grep (no hand-maintained control list); §5 rows trace to the run's real VDR evidence files (never synthetic scanner names) — a disabled scanner is listed, never omitted (Q4), and no VDR evidence → a single `REQUIRES-OPERATOR-INPUT` §5 row (never a fabricated scanner); §3 quotes the FedRAMP ConMon Strategy Guide v3.2 §3.1 verbatim and marks the monthly cadence `REQUIRES-OPERATOR-INPUT-VERIFY` against the live Playbook (the clean-room tree does not reproduce the Playbook's prose); §8 deviation-request process + §10 team roster + §11 escalation SLAs are operator-supplied via `config.yaml: conmon.*`, with the §11 default the published FedRAMP Rev5 / CISA BOD 22-01 baseline (KEV 21d / Critical 30d / High 30d / Moderate 90d / Low 180d, sorted shortest-first, stricter-of-FedRAMP-vs-KEV-due-date note); the document is fully deterministic (UUID = `deterministicUuid('conmon-strategy:'+systemId+':'+runId)`; §13 + the provenance footer cite the `out/ssp.json` SHA-256) so identical inputs produce a byte-identical `.docx`, its integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519). Open questions resolved: Q1 §4 Automated column derived from the real KSI scope; Q2/Q3 §11 sorted SLA-ascending with the stricter-wins note; Q4 §5 always emits a per-scanner row; Q5 §13 + footer cite the SSP SHA-256. Spec reconciliation: the per-slice §7 fixture names + flat scanner schema are stale — the real VDR collector registers as `KSI-AFR-VDR` and emits `KSI-AFR-VDR.json` with scanners as `providers[].findings[]` (LOOP-C-RISKS C-C6-7); the shared `core/docx-primitives.ts` remains unextracted — 12 emitters now carry local OOXML constants (C-C6-8 / C-X-1).

### Added — LOOP-C.C5: FIPS 199 security-categorization worksheet `.docx` emitter (RA-2 / SC-7)

Shipped the FIPS 199 security-categorization worksheet — the signed-by-bundle `out/fips199.docx` (RA-2) that shows the high-water-mark work behind the SSP `system-characteristics.security-impact-level`. New `core/fips199-emit.ts` (pure `computeOverallSC` / `validateInformationType` / `crossReferenceSsp` / `buildFips199BodyXml` / `renderFips199Docx` + disk `emitFips199Docx`) computes the system-level Security Category as the FIPS 199 §3 high-water-mark across the operator-supplied information types (§3), renders the `SC = {(confidentiality, X), (integrity, Y), (availability, Z)}` formula (§4), and cross-checks the result against the real emitted `out/ssp.json` — §4.1 reports CONSISTENT / MISMATCH with a per-objective agreement table. New `core/fips199-types.ts` exports the NIST SP 800-60 Vol. 2 Rev. 1 SaaS-relevant information-type subset (Appendix C — 21 codes: C.2.x Services Delivery Support, C.3.1–C.3.4 Government Resource Management, and the full C.3.5.x Information and Technology Management family) as NIST-published constants (REO Rule 3 allowed exception) with a pinned `SOURCE_VERSION`. Extended `core/submission-bundle.ts` (WELL_KNOWN role `fips199-docx`), `core/orchestrator.ts` (`--fips199` flag + `CLOUD_EVIDENCE_FIPS199` env + the repeatable `--fips199-info-type "code:name:c:i:a:rationale"` flag + a `config.yaml#fips199.*` section, dispatched after the PTA/PIA emit and before signing), and `config.yaml` (new `fips199:` section).

Verification: typecheck clean; the cloud-evidence suite rose 1474 → 1495 (+21: 16 in `tests/core/fips199-emit.test.ts` — the 12 per-slice §8 tests plus unknown-code / sample-fixture / log-event / `impactToken` cases — and 5 in `tests/core/fips199-types.test.ts`), 100% passing; `npm run check:reo` returns 0 (lint:no-stubs 0 violations, check:provenance OK, coverage-regression SKIP with no local `out/inventory-coverage.json`, ssdf-no-silent-pass OK). A smoke run emitted a valid `fips199.docx` (6 OOXML parts, `unzip -t` clean; overall MODERATE; SSP cross-reference CONSISTENT).

Statutory / regulatory drivers (verbatim from the per-slice §2): FIPS PUB 199 (2004-02) §3 — the LOW ("could be expected to have a limited adverse effect on organizational operations, organizational assets, or individuals"), MODERATE ("serious adverse effect"), and HIGH ("severe or catastrophic adverse effect") potential-impact definitions, the loss definitions ("A loss of confidentiality is the unauthorized disclosure of information…"), the SC formula, and the high-water-mark practice ("The generally accepted practice is to use the 'high water mark' over the security impact levels…") — all quoted verbatim in §1.2 / §2; FIPS PUB 200 (2006-03) as the bridge to the SP 800-53 baseline; NIST SP 800-60 Vol. 1 Rev. 1 §3.1 (information-type identification) + Vol. 2 Rev. 1 Appendix C (the information-type catalogue); NIST SP 800-53 Rev. 5 RA-2 ("a. Categorize the system…; b. Document the security categorization results…; c. Verify that the authorizing official or authorizing official designated representative reviews and approves the security categorization decision.").

REO compliance: the overall SC traces to a real `computeOverallSC()` invocation over operator-supplied types — no hardcoded moderate/moderate/moderate default; zero information types → a `REQUIRES-OPERATOR-INPUT` §3 row with the SP 800-60 V2 selection guidance verbatim (`ready_for_signature=false`); invalid impact values throw `Fips199ImpactError` (integrity + availability must be low/moderate/high — `n/a` is permitted only for confidentiality, FIPS 199 §3); a code outside the SP 800-60 V2 subset is accepted with an `UNKNOWN-TYPE-CODE` annotation; the §6 RA-2.c approver signature cells stay `REQUIRES-OPERATOR-INPUT` (the toolkit never auto-signs a human attestation, REO Rule 1.10); the document is fully deterministic (UUID = `deterministicUuid('fips199:'+systemId+':'+runId)`; the SSP provenance is a content SHA-256) so identical inputs produce a byte-identical `.docx`, its integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519), the same coverage `cmp.docx` receives. Spec reconciliation: the real emitted `security-impact-level` is a per-objective object (`security-objective-{confidentiality,integrity,availability}`) plus a `security-sensitivity-level` string, not the per-slice-§schemas single-string enum — `crossReferenceSsp` reads the object shape (a richer per-objective comparison) and still tolerates the bare string (LOOP-C-RISKS C-C5-7); the shared `core/docx-primitives.ts` remains unextracted — 11 emitters now carry local OOXML constants (C-C5-8 / C-X-1).

### Added — LOOP-C.C4: Privacy Threshold Analysis (PTA) + Privacy Impact Assessment (PIA) `.docx` emitters (PT-2 / PT-3 / PT-6 / AR-2)

Shipped the fourth LOOP-C (Document Template Pack) slice end to end — two dependency-free, deterministic Word documents that close a real FedRAMP 20x transcription gap (CSPs currently hand-author the privacy determination from a blank page every assessment cycle). New `core/pta-pia-emit.ts` (`buildPtaBodyXml`/`buildPiaBodyXml`/`renderPtaPiaDocx`/`emitPtaPiaDocx`) renders `out/pta.docx` **always** — a 4-section Privacy Threshold Analysis (1 System Overview, 2 PTA Determination [the five FedRAMP A04 threshold questions], 3 PII Inventory Evidence, 4 Determination & Signature) — and `out/pia.docx` **conditionally** — a 9-section Privacy Impact Assessment (1 Authority & Purpose [PT-2], 2 Categories & Sources [PT-3], 3 Sharing & Use [PT-3], 4 Notice & Consent [PT-6 SORN], 5 Access & Correction, 6 Retention & Disposal, 7 Safeguards, 8 Privacy Risk Assessment [NIST Privacy Framework v1.0 crosswalk], 9 Signature) — emitted only when the PTA determination is positive OR the operator sets `piaForceMode='always-emit'`. The PTA §3 PII-inventory-evidence table auto-derives from the **real** `out/inventory.json`: `readPiiInventory` walks every asset, selects those tagged `data_classification ∈ {pii, phi}` (Q1 `collectsPII` auto-derives from their presence), and renders one evidence row per PII/PHI asset. Resource names are **redacted** (per-slice Risk 3 — the last path/`:` segment is masked to `***` with a stable `ref:<sha8>` correlation hash appended) so the privacy document never itself leaks PII; a `REQUIRES-OPERATOR-INPUT-VERIFY` warning fires when NO asset carries any classification tag (Risk 2 — a PTA-negative determination cannot be trusted on untagged inventory). The determination logic emits the PIA iff any of the five threshold questions is affirmative (or the operator forces it); `piaForceMode='never-emit'` suppresses the PIA even on a positive PTA and renders a §4 "verify with the privacy officer" warning (Risk 4). Every PIA narrative — authority, purposes, PII categories, sources, sharing, consent, SORN reference, access/correction, retention, disposal, safeguards, and the §8 risk assessment — defaults to a verbatim `REQUIRES-OPERATOR-INPUT` marker; the emitter **never invents** PII categories such as "name, email, SSN" (REO Rule 4 + per-slice REO check). The §4 (PTA) and §9 (PIA) signature cells stay `REQUIRES-OPERATOR-INPUT` (the toolkit never auto-signs a human privacy attestation — REO Rule 1.10). When an inventoried asset carries a `phi` tag, PIA §8 adds an advisory `REQUIRES-OPERATOR-INPUT-VERIFY` note that HIPAA obligations may apply in addition to (and outside the scope of) FedRAMP authorization (Open Q3).

Wiring: orchestrator `--pta-pia` (env `CLOUD_EVIDENCE_PTA_PIA`) dispatches the emitter AFTER the IRP emit and BEFORE signing so both docs are covered by the submission bundle; structured input flows from a new `config.yaml#privacy.*` section (`pia_force_mode` + the `pta` / `pia` objects, whose keys are the `PtaResponses` / `PiaResponses` field names passed straight through); `core/submission-bundle.ts` gains two WELL_KNOWN roles (`pta-docx` + a `conditional:true` `pia-docx`, with a new `conditional?` field on `WellKnownArtifact` so a missing PIA is never flagged as an absent required artifact). Both documents are fully deterministic — no wall-clock time (UUID seeds are `pta:${systemId}:${runId}` and `pia:${systemId}:${runId}`; the template retrieval date is a pinned constant), so identical inputs produce byte-identical `.docx`. Integrity is anchored by the signed submission-bundle `INDEX.json` (SHA-256 + Ed25519), the same coverage `irp.docx`/`iscp.docx`/`cmp.docx`/`roe.docx`/`ssp.docx` receive.

Statutory / regulatory drivers (cited verbatim in the module header + document bodies): NIST SP 800-53 Rev. 5 **PT-2** ("a. Determine and document the [Assignment: organization-defined authority] that permits the [Assignment: organization-defined processing] of personally identifiable information; and b. Restrict the [Assignment: organization-defined processing] of personally identifiable information to only that which is authorized."), **PT-3** ("a. Identify and document the [Assignment: organization-defined purpose(s)] for processing personally identifiable information; b. Describe the purpose(s) in the public privacy notices and policies of the organization; c. Restrict the [Assignment: organization-defined processing] of personally identifiable information to only that which is compatible with the identified purpose(s); d. Monitor changes in processing personally identifiable information ..."), **PT-6** ("For systems that process information that will be maintained in a Privacy Act system of records: a. Draft and publish System of Records Notices in the Federal Register ...; b. Keep System of Records Notices accurate, up-to-date, and scoped ...; c. Review System of Records Notices [Assignment: organization-defined frequency]."), and **AR-2** (Privacy Impact and Risk Assessment) — https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final; FedRAMP SSP Attachment A04 PIA Template (Rev4, section structure mirrored) — https://www.fedramp.gov/assets/resources/templates/SSP-A04-FedRAMP-PIA-Template.docx (FedRAMP help-desk article 28907995813275: "There are no current plans to provide a Rev. 5 PTA/PIA template for CSPs to complete." — Rev4 structure used with Rev5 PT-family control IDs); OMB Memorandum M-03-22 (E-Government Act §208) — https://www.whitehouse.gov/wp-content/uploads/legacy_drupal_files/omb/memoranda/2003/m03_22.pdf; NIST Privacy Framework v1.0 (2020-01-16) — https://nvlpubs.nist.gov/nistpubs/CSWP/NIST.CSWP.01162020.pdf.

Verification: `npm run typecheck` clean; `npm test` 1474/1474 passing (was 1457 after C.C3 — +17: 16 PTA/PIA tests [14 per the C.C4.md §8 spec + an error-guard test + a PII-redaction test] + 1 log-event test, clearing the universal ≥15-new-tests floor); `npm run check:reo` returns 0 (lint:no-stubs + check:provenance + check:coverage-regression + check:ssdf-no-silent-pass all green); both `.docx` pass `unzip -t` (valid OOXML, 6 parts) and a smoke run confirmed the raw resource names never appear in the emitted document. REO compliance: every §3 evidence row traces to a real `inventory.json` `data_classification` tag with the resource name redacted (no PII leak, no substituted values); PIA categories are empty (`REQUIRES-OPERATOR-INPUT`) when not operator-supplied (never invented); the determination is never fabricated; signatures are never auto-signed. Files created: `core/pta-pia-emit.ts`, `tests/core/pta-pia-emit.test.ts`, and 3 fixtures under `tests/core/fixtures/pta-pia/` (`inventory.with-pii.json`, `inventory.no-pii.json`, `config.full.yaml`). Files modified: `core/orchestrator.ts` (flag/env/config/dispatch), `core/submission-bundle.ts` (2 roles + `conditional?` field). Spec reconciliations recorded in `docs/loops/LOOP-C-RISKS.md` C-C4-7 (`data_classification` is a free-form asset tag read defensively — snake_case, camelCase, and common tag keys — NOT the hard `pii`/`phi` enum the per-slice §7 assumed, so `core/inventory-emit.ts` needed no modification) and C-C4-8 (the shared `core/docx-primitives.ts` was again not extracted — now 10 emitters to migrate when C-X-1 lands). §10 open questions Q1–Q5 all resolved (see the C.C4 scope note in `docs/STATUS.md`); the tracker `privacy_responses` capture is deferred to LOOP-E (C-C4-9, C-X-13 posture).

### Added — LOOP-C.C3: Incident Response Plan (IRP, IR-8) + Incident Response Test After-Action Report (IR-3) `.docx` emitters

Shipped the third LOOP-C (Document Template Pack) slice end to end — two dependency-free, deterministic Word documents that close a real FedRAMP 20x transcription gap (CSPs currently hand-author the IR-8 plan and its annual test report from a blank page every assessment cycle). New `core/irp-emit.ts` (`buildIrpBodyXml`/`renderIrpDocx`/`emitIrpDocx`) renders `out/irp.docx` — an 11-section Incident Response Plan (1 Introduction & Scope, 2 Roles & Responsibilities, 3 Incident Classification, 4 Detect, 5 Respond, 6 Communications Plan, 7 External Contacts, 8 Escalation Matrix, 9 Reporting Requirements, 10 Lessons Learned, 11 Plan Maintenance & Testing) structured per NIST SP 800-61 Rev. 3's CSF 2.0 life-cycle. The §4 Detect table auto-fills from the **real** signed `KSI-INR-RIR.json` evidence: `readInrRirEvidence` flattens every provider block's `findings[]` into one detection-source row (rule + ACTIVE/GAP + `current_state.summary`) and derives a coverage percent from the pass ratio, surfacing a WARNING paragraph when coverage is below 95% so the plan never papers over a detection gap. §5 Respond renders the six CSF 2.0 Functions (Govern / Identify / Protect / Detect / Respond / Recover) as sub-sections; the `--irp-spec-version=800-61r2` flag swaps in the withdrawn four-phase model for reviewers still on it. §9 Reporting Requirements bakes the FedRAMP Incident Communications Procedures notification SLAs (1 hour to the PMO + 1 hour to the impacted agency + 4 hours to CISA US-CERT for federal-data incidents) as `REQUIRES-OPERATOR-INPUT-VERIFY` baseline rows; §3 classification levels, §7 external contacts (FedRAMP `info@fedramp.gov` + CISA `report@cisa.gov` published role-based addresses — no personal PII), and the severity-sorted §8 escalation matrix likewise default to verify-marked FedRAMP baselines. Every operator narrative (IR-team roster, communications plan) defaults to a verbatim `REQUIRES-OPERATOR-INPUT` marker. New `core/irp-test-aar.ts` renders `out/irp-test-aar.docx` — a 7-section After-Action Report (Test Overview, Scenarios Executed, Timing Metrics, Outcomes, Lessons Learned, Recommendations & Action Items, Sign-off) whose central artifact is the 5-phase timing matrix (detection → response → containment → eradication → recovery, minutes since incident onset); test scenarios + lessons learned are operator-supplied (never fabricated — an empty `scenarios[]` yields a single `REQUIRES-OPERATOR-INPUT` row), failed scenarios are flagged in §4, high/critical lessons route to a §6 POA&M footer note, and the §7 sign-off block's signature/date cells stay `REQUIRES-OPERATOR-INPUT` (the toolkit never auto-signs a human attestation — REO Rule 1.10).

Wiring: orchestrator `--irp` / `--irp-test-aar` (envs `CLOUD_EVIDENCE_IRP` / `CLOUD_EVIDENCE_IRP_TEST_AAR`) + `--irp-spec-version` (env `CLOUD_EVIDENCE_IRP_SPEC_VERSION`) dispatch the emitters AFTER the ISCP emit and BEFORE signing (IRP before the AAR so its `out/irp.docx` SHA-256 anchor resolves); structured input flows from a new `config.yaml#irp.*` section (spec_version / team_roster / escalation / external_contacts / communications / classification_levels + `irp.test.*` scenarios/timing/lessons/participants); `core/submission-bundle.ts` gains two WELL_KNOWN roles (`irp-docx`, `irp-test-aar-docx`). Both documents are fully deterministic — no wall-clock time (UUID seeds are `irp:${systemId}:${runId}` and `irp-test-aar:${runId}:${testDate}`; `testType`/`testDate` default to `REQUIRES-OPERATOR-INPUT` rather than `new Date()`), so identical inputs produce byte-identical `.docx`. Integrity is anchored by the signed submission-bundle `INDEX.json` (SHA-256 + Ed25519), the same coverage `iscp.docx`/`cmp.docx`/`roe.docx`/`ssp.docx` receive. `docs/OPERATOR-GUIDE.md` §3/§4/§7 document the new flags/envs/outputs.

Statutory / regulatory drivers (cited verbatim in the module headers + document bodies): NIST SP 800-53 Rev. 5 **IR-8** ("Develop an incident response plan that: 1. Provides the organization with a roadmap for implementing its incident response capability; ... 5. Defines reportable incidents; 6. Provides metrics for measuring the incident response capability within the organization; ... 9. Is reviewed and approved by [Assignment: organization-defined personnel or roles]; 10. Explicitly designates responsibility for incident response to [Assignment: organization-defined entities, personnel, or roles]."), **IR-3** ("Test the effectiveness of the incident response capability for the system [Assignment: organization-defined frequency] using the following tests: [Assignment: organization-defined tests]."), **IR-4** (Incident Handling) and **IR-6** (Incident Reporting) — https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final; NIST SP 800-61 Rev. 3 §2.1 (quoted verbatim in §1.3): "This publication provides recommendations for managing incident response throughout the incident lifecycle, structured around the NIST Cybersecurity Framework (CSF) 2.0 Functions: Govern, Identify, Protect, Detect, Respond, and Recover." — https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-61r3.pdf (Rev. 2 was officially withdrawn April 2025); FedRAMP Incident Communications Procedures (CSP notification SLAs) — https://www.fedramp.gov/assets/resources/documents/CSP_Incident_Communications_Procedures.pdf.

Verification: `npm run typecheck` clean; `npm test` 1457/1457 passing (was 1433 after C.C2 — +24: 14 IRP + 10 AAR, ≥ the 23 spec tests); `npm run check:reo` returns 0 (lint:no-stubs + check:provenance + check:coverage-regression + check:ssdf-no-silent-pass all green); both `.docx` pass `unzip -t` (valid OOXML, 6 parts). REO compliance: every §4 detection row traces to a real KSI-INR-RIR finding (no fabricated "logging is configured" rows); §5/§9 quote NIST SP 800-61 Rev. 3 §2.1 + IR-8/IR-3 verbatim; the AAR's timing metrics come only from operator input; the sign-off is never auto-signed. Files created: `core/irp-emit.ts`, `core/irp-test-aar.ts`, `tests/core/irp-emit.test.ts`, `tests/core/irp-test-aar.test.ts`, and 2 fixtures under `tests/core/fixtures/irp/` (`KSI-INR-RIR.json`, `config.sample.yaml`). Files modified: `core/orchestrator.ts`, `core/submission-bundle.ts`, `config.yaml`. Spec reconciliations recorded in `docs/loops/LOOP-C-RISKS.md` C-C3-7 (the INR-RIR evidence file is the real collector output `KSI-INR-RIR.json` not the §7-named `.signed.json`, and the real envelope carries detection capabilities as `providers[].findings[]` with a top-level `collected_at`, not the §5-assumed flat `evidence[].detection_source`/`coverage_percent` — mirroring C-C2-7/C-C2-9) and C-C3-8 (the shared `core/docx-primitives.ts` was again not extracted — now 8 emitters to migrate when C-X-1 lands). §10 open questions Q1–Q6 all resolved (see the C.C3 scope note in `docs/STATUS.md`); the tracker `irp_tests` capture is deferred to LOOP-E.E7 (C-C3-9).

### Added — LOOP-C.C2: Information System Contingency Plan (ISCP, CP-2) + Contingency Plan Test After-Action Report (CP-4) `.docx` emitters

Shipped the second LOOP-C (Document Template Pack) slice end to end — two dependency-free, deterministic Word documents that close a real FedRAMP 20x transcription gap (CSPs currently hand-author both from the blank FedRAMP SSP Appendix G template every assessment cycle). New `core/iscp-emit.ts` (`buildIscpBodyXml`/`renderIscpDocx`/`emitIscpDocx`) renders `out/iscp.docx` — 6 sections (Introduction & Scope, Concept of Operations, Activation & Notification, Recovery, Reconstitution, Plan Maintenance) + 6 appendices (A Personnel Contact List, B Vendor/Subprocessor Contacts, C Detailed Recovery Procedures, D Alternate Site Procedures, E System Validation Test Plan, F Contingency Plan Test Report) mirroring the FedRAMP SSP Appendix G ISCP Template. The §4.2 Recovery-evidence table auto-fills from the real signed RPL-family KSI files (`readRplEvidence` reads `KSI-RPL-ABO/TRC/RRO/ARP.json` — one row per KSI carrying pass/fail + `collected_at` + a SHA-256 evidence citation); §2.1 components and Appendix B vendor contacts **compose** the existing readers (`readInventoryComponents`/`groupComponents` imported from `core/cmp-emit.ts`, `readSubprocessorContacts` reads the real `subprocessor-inventory.json`); every recovery narrative defaults to a verbatim `REQUIRES-OPERATOR-INPUT` marker. New `core/iscp-test-aar.ts` renders `out/iscp-test-aar.docx` — 6 sections (Test Overview, Scenarios Executed, Test Results Summary, Lessons Learned, Recommendations & Action Items, Sign-off) per Appendix G Appendix F; test scenarios + lessons learned are operator-supplied (never fabricated — an empty `scenarios[]` yields a single `REQUIRES-OPERATOR-INPUT` row naming the runId), failed scenarios are flagged in §3, high/critical lessons route to a POA&M footer note, and the §6 sign-off block's signature/date cells stay `REQUIRES-OPERATOR-INPUT` (the toolkit never auto-signs a human attestation).

Wiring: orchestrator `--iscp` / `--iscp-test-aar` (envs `CLOUD_EVIDENCE_ISCP` / `CLOUD_EVIDENCE_ISCP_TEST_AAR`) + four value flags (`--iscp-rto-hours`, `--iscp-rpo-hours`, `--iscp-test-date`, `--iscp-test-type`) dispatch the emitters AFTER the CMP emit and BEFORE signing (ISCP before the AAR so its `out/iscp.docx` SHA-256 anchor resolves); structured input flows from a new `config.yaml#iscp.*` section (rto/rpo/alternate_site/activation/rosters/vendor_contacts + `iscp.test.*` scenarios/lessons/participants); `core/submission-bundle.ts` gains two WELL_KNOWN roles (`iscp-docx`, `iscp-test-aar-docx`). Both documents are fully deterministic — no wall-clock time (UUID seeds are `iscp:${systemId}:${runId}` and `iscp-test-aar:${runId}:${testDate}`; `testDate` defaults to `REQUIRES-OPERATOR-INPUT` rather than `new Date()`), so identical inputs produce byte-identical `.docx`. Integrity is anchored by the signed submission-bundle `INDEX.json` (SHA-256 + Ed25519), the same coverage `cmp.docx`/`roe.docx`/`ssp.docx` receive.

Statutory / regulatory drivers (cited verbatim in the module headers + document bodies): NIST SP 800-53 Rev. 5 **CP-2** ("Develop a contingency plan for the system that: a. Identifies essential mission and business functions and associated contingency requirements; b. Provides recovery objectives, restoration priorities, and metrics; ..."), **CP-4** ("Test the contingency plan for the system [Assignment: organization-defined frequency] ...; Review the contingency plan test results; Initiate corrective actions, if needed; ..."), **CP-9** (System Backup) and **CP-10** (System Recovery and Reconstitution) — https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final; NIST SP 800-34 Rev. 1 §3.1 seven-step contingency-planning process (quoted verbatim in ISCP §1.3) and §3.6 test-type definitions — https://csrc.nist.gov/pubs/sp/800/34/r1/upd1/final; FedRAMP SSP Appendix G ISCP Template (section structure mirrored) — https://www.fedramp.gov/assets/resources/templates/SSP-Appendix-G-Information-System-Contingency-Plan-(ISCP)-Template.docx.

Verification: `npm run typecheck` clean; `npm test` 1433/1433 passing (was 1409 after C.C1 — +24: 14 ISCP + 10 AAR, ≥ the 23 spec tests); `npm run check:reo` returns 0 (lint:no-stubs + check:provenance + check:coverage-regression + check:ssdf-no-silent-pass all green); both `.docx` pass `unzip -t` (valid OOXML). REO compliance: every §4.2 row traces to a real signed evidence file (no fabricated "backup is configured" rows); Appendix B traces to real subprocessor rows (contact/phone stay `REQUIRES-OPERATOR-INPUT` because the SA-9 inventory carries no such fields); AAR scenarios come only from operator input. Files created: `core/iscp-emit.ts`, `core/iscp-test-aar.ts`, `tests/core/iscp-emit.test.ts`, `tests/core/iscp-test-aar.test.ts`, and 6 fixtures under `tests/core/fixtures/iscp/`. Files modified: `core/orchestrator.ts`, `core/submission-bundle.ts`. Spec reconciliations recorded in `docs/loops/LOOP-C-RISKS.md` C-C2-7..11 (RPL file name `.json` not `.signed.json`; Appendix B source `subprocessor-inventory.json` not `subprocessors.json`; envelope `collected_at`/`rollup.pass` not `findings[].metadata`; docx-primitives still not extracted; tracker capture deferred to LOOP-E.E7).

### Added — LOOP-C.C1: Configuration Management Plan (CM-9 `.docx` emitter)

Shipped the Configuration Management Plan end to end — the first LOOP-C (Document Template Pack) slice. New `core/cmp-emit.ts` renders `out/cmp.docx`, an 11-section CM-9 Configuration Management Plan, via the dependency-free store-only OOXML writer (`core/zip.ts`) — no external Word library, no runtime network. The renderer is pure (`buildCmpBodyXml` → `renderCmpDocx`) with a disk emitter (`emitCmpDocx`); §4 Configuration Items are auto-derived from the real `out/inventory.json` (CM-8) via `readInventoryComponents` + `groupComponents` (grouped by `(provider, assetType)` with per-group asset counts + distinct locations); §7 Configuration Monitoring lists the real KSI domains grepped from `core/ksi-map.ts` via `readKsiScope` (44 domains; the emitter throws `CmpKsiScopeError` when fewer than 20 resolve, mirroring the RoE emitter's minimum-scope guard). Every process-narrative section — §3 CCB roster (the four NIST SP 800-128 §2.1 roles), §6 change control (CM-3/CM-4), §8 change windows, §9 rollback authority, §10 CM tooling — defaults to a verbatim `REQUIRES-OPERATOR-INPUT` marker; §10 tooling inferred from the inventory provider set carries the distinct `REQUIRES-OPERATOR-INPUT-VERIFY` marker so the operator confirms actual use. The document is fully deterministic (no wall-clock time): a `docProps/core.xml` `<dc:title>` carries `deterministicUuid('cmp:'+systemId+':'+runId)` and the §1 provenance cites the inventory content SHA-256, so identical inputs yield a byte-identical `.docx`.

Extended `core/submission-bundle.ts` (WELL_KNOWN role `cmp-docx` → `cmp.docx`), `core/orchestrator.ts` (`--cmp` flag + env `CLOUD_EVIDENCE_CMP`, plus the four value flags `--cmp-approval-narrative` / `--cmp-rollback-authority` / `--cmp-change-windows` / `--cmp-baseline-config-href` with matching envs and `config.yaml#cmp.*` fallbacks; a `Config.cmp` section for the CCB roster + tooling; a dispatch block that runs after the RoE emit and before signing so `cmp.docx` is present in `out/` when the submission bundle + run manifest are built; a `ledger.record('cmp.emit', …)` run-ledger entry), and `config.yaml` (a documented `cmp:` section). Added `tests/core/cmp-emit.test.ts` (18 tests) with fixtures `tests/core/fixtures/cmp/{inventory.sample.json, config.sample.yaml}`. Documented the new flags/envs/output in `docs/OPERATOR-GUIDE.md` §3/§4/§7.

**Statutory / regulatory drivers** (per the per-slice doc §2, cited verbatim): NIST SP 800-53 Rev. 5 CM-9 (Configuration Management Plan) — "Develop, document, and implement a configuration management plan for the system that: a. Addresses roles, responsibilities, and configuration management processes and procedures; b. Establishes a process for identifying configuration items throughout the system development life cycle and for managing the configuration of the configuration items; c. Defines the configuration items for the system and places the configuration items under configuration management; d. Is reviewed and approved by [Assignment: organization-defined personnel or roles]; e. Protects the configuration management plan from unauthorized disclosure and modification." NIST SP 800-128 §2.1 — "The roles and responsibilities for SecCM should be clearly identified within the organization. These roles often include configuration control board, change initiator, change implementer, and change approver." (Appendix D drives the §1–§11 outline.) NIST SP 800-53 Rev. 5 CM-3 (Configuration Change Control), CM-4 (Security Impact Analyses), CM-8 (System Component Inventory), CM-2 (Baseline Configuration). FedRAMP publishes no CMP template (help-desk confirmed June 2026), so C.C1 closes a real 3PAO-flagged gap.

**REO compliance:** every §4 Configuration Item traces to a real asset in `inventory.json` (the emitter never invents component types); §7 traces to the live `ksi-map.ts`; §3/§6/§8/§9/§10 default to verbatim operator-input markers (no Lorem-Ipsum, no TODO, no sample workflow language); the CSP/operator is author-of-record (the system never auto-approves — CM-9.d). `cmp.docx` is a printable companion whose integrity is anchored by the signed submission-bundle `INDEX.json` (SHA-256 + Ed25519 over every included file) — `core/sign.ts` was not modified (`.docx` is not in `SIGNED_EXTENSIONS`, the same posture as `roe.docx`/`ssp.docx`). Spec reconciliations recorded in `docs/loops/LOOP-C-RISKS.md` C-C1-6 (the LOOP-C-SPEC §4 pre-slice shared `core/docx-primitives.ts` was NOT extracted — the per-slice §7 file list scopes C.C1 to `cmp-emit.ts` and the four shipped docx emitters keep local OOXML constants; the C-X-1 refactor is deferred), C-C1-7 (`deterministicUuid` is single-arg — seed composed as `cmp:${systemId}:${runId}`), C-C1-8 (`.docx` integrity via the bundle INDEX.json, not per-file `.sig`; the "grep timestamp" provenance reconciled to `runId` for byte-determinism). §10 open questions Q1–Q5 all resolved (see the per-slice doc). Verification: typecheck clean, 1391 → 1409 tests passing (+18), `npm run check:reo` green (G1 lint:no-stubs 0 violations, G3 provenance OK; G2 coverage-regression skips with no local `out/`).
