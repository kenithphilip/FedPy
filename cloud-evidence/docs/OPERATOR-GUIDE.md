...
| `--conmon-strategy` | `out/conmon-strategy.docx` | LOOP-C.C6. Continuous Monitoring Strategy + Plan (CA-7 / CA-7(1) / PM-31) — 13 sections. §4 Controls Under Continuous Monitoring is auto-derived from the live `core/ksi-map.ts` (one row per KSI, with an Automated column derived from the KSI scope: CLOUD → Yes, HYBRID → Partial, PROCESS → No); §5 Vulnerability Scanning is auto-derived from the run's real `KSI-*VDR*` evidence (one row per scanner/provider block — scanner name, detection-enabled, findings/KEV counts, last-collected, evidence SHA-256; a disabled scanner is listed, never omitted); §3 quotes the FedRAMP ConMon Strategy Guide v3.2 §3.1 verbatim + cites the ConMon Playbook v1.0; §9 reporting endpoint keys off the impact level (Low/Moderate → USDA Connect.gov, High → agency-direct); §11 escalation defaults to the FedRAMP Rev5 / CISA BOD 22-01 baseline (KEV 21d / Critical 30d / High 30d / Moderate 90d / Low 180d, sorted shortest-first). Team roster (§10) + escalation (§11) + deviation process (§8) fall back to `REQUIRES-OPERATOR-INPUT` / `REQUIRES-OPERATOR-INPUT-VERIFY`. Deterministic; integrity anchored by the signed submission-bundle INDEX.json. Structured input via `config.yaml: conmon.*`. Runs after `--fips199`, before signing. (env: `CLOUD_EVIDENCE_CONMON_STRATEGY`) |
| `--rms` | `out/rms.docx` | LOOP-C.C7. Risk Management Strategy (PM-9) — 11 sections. The organization-level risk-management strategy (framing → assessing → responding → monitoring per NIST SP 800-39 §2) that sits above the SSP + POA&M in the SP 800-39 §2.1 three-tier hierarchy. §5 Risk Register Reference auto-links to `out/risk-register.json` (LOOP-B.B5, RA-3) and embeds its summary (entries/open/high-inherent/by-source + SHA-256); §6 Risk Acceptance Policy summarizes the `.risk-acceptances.json` (B.B3) + `.compensating-controls.json` (B.B4) snapshots; §10 POA&M Summary auto-counts severities (critical/high/medium/low/info), overdue open items, and the oldest-open-finding age from `out/poam.json` (LOOP-A.A1). Each absent input degrades to `REQUIRES-OPERATOR-INPUT` with a "Generate via LOOP-B / --oscal-poam" cross-link (never blocks); an unknown POA&M severity throws `PoamSeverityError`. §4 renders the NIST SP 800-39 Accept/Avoid/Mitigate/Transfer 4-row matrix; §7 cross-links to the C.C6 ConMon Strategy when present. Risk tolerance (§8) + executive oversight (§9) fall back to `REQUIRES-OPERATOR-INPUT`. Deterministic (the §10 "now" derives from the POA&M `metadata.last-modified`, never a wall clock); integrity anchored by the signed submission-bundle INDEX.json. Structured input via `config.yaml: rms.*`. Runs after `--conmon-strategy`, before signing. (env: `CLOUD_EVIDENCE_RMS`) |
| `--auth-cover-letter` | `out/auth-request-cover-letter.docx` | LOOP-C.C8. Authorization-request cover letter / package transmittal (PM-10) — the CSP-side formal cover letter that ACCOMPANIES a FedRAMP authorization package (distinct from the AO-issued ATO letter, which is the AO's RESPONSE). 7 sections + letterhead + addressee (FedRAMP Agency Authorization Playbook v4.1 §3 structure). §4 Package Contents auto-enumerates the run's signed `out/INDEX.json` (LOOP-A.A4) — one row per artifact with filename + role + the REAL sha256-short (first 12 hex chars) + bytes, alphabetically sorted; §3 3PAO Statement reads `out/ap.json` metadata (LOOP-A.A2) for the assessing 3PAO org + AP finalization date. Each absent LOOP-A input degrades to `REQUIRES-OPERATOR-INPUT` with a "run with --submission-bundle / --oscal-ap" cross-link. §1 quotes NIST SP 800-53 Rev. 5 PM-10(a) verbatim + cites SP 800-37 Rev. 2 §3.6. Exec signatory (§7), CSP address (letterhead), AO addressee (with optional delegated designee), and 3PAO lead come from `config.yaml: auth_request.*` + `config.yaml: org.address`; the exec signature block is NEVER auto-signed. When co-run with `--submission-bundle` the orchestrator sequences INDEX-build → cover-letter → bundle-pack (a pre-pass materializes INDEX.json; the final bundle re-packs with the letter enclosed), so the letter ships inside the tarball. Deterministic; integrity anchored by the signed submission-bundle INDEX.json. Runs after signing / with the bundle. (env: `CLOUD_EVIDENCE_AUTH_COVER_LETTER`) |
| `--auth-request-type <t>` | `initial-ato` \| `continued-ato` \| `reauthorization` | LOOP-C.C8. Requested authorization action for `--auth-cover-letter` (§2 Request Summary). Default `initial-ato`. Also settable via `config.yaml: auth_request.ato_type`; the CLI flag wins. (env: `CLOUD_EVIDENCE_AUTH_REQUEST_TYPE`) |
| `--baseline-config` | `out/baseline-config.docx` | LOOP-C.C9. Baseline Configuration document (NIST SP 800-53 Rev. 5 CM-2 / CM-2(2) / CM-2(7)) — the CSP's documented, approved baseline of record, distinct from the CM-8 inventory (what components exist) and the AFR Secure Configuration Guide (the recommended config). 8 sections. §3 Baseline Configuration Items groups the run's real `out/inventory.json` by (cloud provider, normalized component class — compute/container/database/storage/network/identity/serverless) with the current image(s) + hardening baseline per group; §4 Reference Architecture is grep-read from `providers/{aws,gcp,azure}/reference-arch.ts` SOURCE (each documented hardening expectation → rule + target + NIST controls), so no cloud SDK client loads at emit time; §5 Deviations from Baseline is a pure diff of the two, each deviation's severity taken from the anchoring reference finding (never defaulted) — two kinds: undocumented-hardening-baseline (compute/container asset with empty `baselineConfig`) and encryption-at-rest-disabled. When `inventory.json` is absent §3/§5 degrade to `REQUIRES-OPERATOR-INPUT`; when no `reference-arch.ts` is readable §4/§5 degrade likewise. §1 quotes NIST SP 800-53 CM-2(a) + NIST SP 800-128 §3.2 verbatim; §7 cross-links to the CMP (`cmp.docx §6`, C.C1). Baseline approver (§8), deviation-log location (§7), and review cadence (§6, default `annually` per CM-2) come from `config.yaml: baseline_config.*`; the §8 approval signature is NEVER auto-signed. Runs BEFORE `--cmp` (auto-resolves the CMP §5 `--cmp-baseline-config-href` cross-link to `baseline-config.docx` when co-emitted) and BEFORE signing so the .docx is bundle-covered. Deterministic; integrity anchored by the signed submission-bundle INDEX.json (each source's SHA-256 cited in the provenance footer). (env: `CLOUD_EVIDENCE_BASELINE_CONFIG`) |
...
| `CLOUD_EVIDENCE_CONMON_STRATEGY` | `0` | LOOP-C.C6. Emit the Continuous Monitoring Strategy + Plan (`out/conmon-strategy.docx`) — CA-7 / CA-7(1) / PM-31. Same as `--conmon-strategy`. (Team roster + escalation SLAs + deviation process + reporting endpoint + collaborative-ConMon are supplied via `config.yaml: conmon.*`.) |
| `CLOUD_EVIDENCE_RMS` | `0` | LOOP-C.C7. Emit the Risk Management Strategy (`out/rms.docx`) — PM-9. Same as `--rms`. §5/§6/§10 auto-derive from the run's real risk corpus (`risk-register.json` B.B5 / the `.risk-acceptances.json` + `.compensating-controls.json` snapshots B.B3/B.B4 / `poam.json` A.A1); risk tolerance + executive oversight are supplied via `config.yaml: rms.*`. |
| `CLOUD_EVIDENCE_AUTH_COVER_LETTER` | `0` | LOOP-C.C8. Emit the authorization-request cover letter (`out/auth-request-cover-letter.docx`) — PM-10. Same as `--auth-cover-letter`. §4 Package Contents auto-enumerates the run's signed `INDEX.json` (A.A4); §3 3PAO statement reads `ap.json` metadata (A.A2); exec signatory + AO addressee + 3PAO lead are supplied via `config.yaml: auth_request.*` + `config.yaml: org.address` and are never auto-signed. |
| `CLOUD_EVIDENCE_BASELINE_CONFIG` | `0` | LOOP-C.C9. Emit the Baseline Configuration document (`out/baseline-config.docx`) — CM-2. Same as `--baseline-config`. §3 Baseline Configuration Items groups the real `inventory.json` (CM-8); §4 Reference Architecture is grep-read from `providers/{aws,gcp,azure}/reference-arch.ts` source; §5 Deviations is a pure diff of the two (severity from the anchoring reference finding, never defaulted). Approver + deviation-log + review-cadence come from `config.yaml: baseline_config.*` and the §8 approval signature is never auto-signed. Runs before `--cmp` (auto-resolves the CMP §5 baseline cross-link) and before signing. |
| `CLOUD_EVIDENCE_AUTH_REQUEST_TYPE` | (none) | LOOP-C.C8. Requested authorization action for the cover letter: `initial-ato` (default) / `continued-ato` / `reauthorization`. Equivalent to `--auth-request-type`. |
...
| `conmon-strategy.docx` | No | `--conmon-strategy` (LOOP-C.C6) | OOXML docx (Continuous Monitoring Strategy + Plan, CA-7 / CA-7(1) / PM-31; §4 Controls Under Continuous Monitoring from the live `core/ksi-map.ts`, §5 Vulnerability Scanning from the run's real `KSI-*VDR*` evidence, §9 reporting endpoint by impact level, §11 escalation SLAs) — LOOP-E reads it as the config for the monthly ConMon runs | printable companion — integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519) |
| `rms.docx` | No | `--rms` (LOOP-C.C7) | OOXML docx (Risk Management Strategy, PM-9; the organization-level framing → assessing → responding → monitoring strategy above the SSP + POA&M; §5 Risk Register Reference from `risk-register.json` B.B5, §6 Risk Acceptance Policy from the `.risk-acceptances.json`/`.compensating-controls.json` snapshots B.B3/B.B4, §10 POA&M Summary from `poam.json` A.A1 — severity histogram + overdue + oldest-open age; §4 Accept/Avoid/Mitigate/Transfer matrix) | printable companion — integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519) |
| `auth-request-cover-letter.docx` | No | `--auth-cover-letter` (LOOP-C.C8) | OOXML docx (CSP-side authorization-request cover letter / package transmittal, PM-10; distinct from the AO-issued ATO letter; §4 Package Contents auto-enumerated from the signed `INDEX.json` with real sha256-short + bytes per artifact, §3 3PAO statement from `ap.json` metadata; FedRAMP Agency Authorization Playbook v4.1 §3 structure; exec signature block never auto-signed) | ships inside `submission-package.tar.gz` — integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519) |
| `baseline-config.docx` | No | `--baseline-config` (LOOP-C.C9) | OOXML docx (CM-2 Baseline Configuration document / baseline of record; distinct from the CM-8 inventory + the AFR Secure Configuration Guide; §3 Baseline Configuration Items grouped from the real `inventory.json` by provider+component-class, §4 Reference Architecture grep-read from `providers/{aws,gcp,azure}/reference-arch.ts` source, §5 Deviations from Baseline a pure diff of the two with severity from the anchoring reference finding; §1 quotes CM-2(a) + SP 800-128 §3.2 verbatim; §7 cross-links `cmp.docx §6`; approval signature never auto-signed) | ships inside `submission-package.tar.gz` — integrity anchored by the signed submission-bundle INDEX.json (SHA-256 + Ed25519); provenance footer cites each source SHA-256 |
