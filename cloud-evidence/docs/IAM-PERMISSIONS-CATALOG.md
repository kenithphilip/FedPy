...
| `logging.ts` | MLA-ALA, MLA-EVC, MLA-LET, MLA-OSM, MLA-RVL, CMT-LMC, INR-RIR | CloudTrail, CloudWatch Logs, S3, Athena, Firehose, SecurityHub, CodeBuild | `cloudtrail:DescribeTrails`, `cloudtrail:GetTrailStatus`, `cloudtrail:GetEventSelectors`, `cloudtrail:GetInsightSelectors`, `logs:DescribeLogGroups`, `logs:DescribeSubscriptionFilters`, `s3:GetBucketPolicy`, `s3:GetBucketEncryption`, `s3:GetBucketVersioning`, `s3:GetObjectLockConfiguration`, `athena:ListWorkGroups`, `firehose:ListDeliveryStreams`, `securityhub:GetFindings`, `securityhub:GetEnabledStandards`, `codebuild:ListProjects` |
...
| `data.ts` | SVC-RUD, SVC-VCM, SVC-VRI | Cloud Storage, KMS, Certificate Manager | `roles/storage.objectViewer` (bucket metadata), `roles/cloudkms.viewer`, `roles/certificatemanager.viewer` |
