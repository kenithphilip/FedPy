...
| `config.ts` (`collectCnaEis`, `collectCnaIbp`) | KSI-CNA-EIS, KSI-CNA-IBP | Resource Graph — `policyresources` (`microsoft.authorization/policyassignments`, `microsoft.policyinsights/policystates`) | `Reader` is sufficient — `policyresources` table is readable to anyone with subscription Reader. |
