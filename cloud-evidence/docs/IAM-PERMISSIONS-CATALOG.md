...
| `backup.ts` (`collectRplAbo`, `collectRplTrc`) | KSI-RPL-ABO, KSI-RPL-TRC | Resource Graph — Resources (`microsoft.recoveryservices/vaults`, `microsoft.dataprotection/backupvaults`) + `RecoveryServicesResources` table (`microsoft.recoveryservices/vaults/backupprotecteditems`, `microsoft.recoveryservices/vaults/backupjobs`) | `Reader` is sufficient; `RecoveryServicesResources` is exposed to Resource Graph under the same Reader RBAC as the `Resources` table. |
