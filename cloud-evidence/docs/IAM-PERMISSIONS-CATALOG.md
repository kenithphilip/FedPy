...
| `network.ts` (`collectCnaUln`, `collectCnaRvp`, `collectSvcSnt`) | KSI-CNA-ULN, KSI-CNA-RVP, KSI-SVC-SNT | Resource Graph — `microsoft.network/networkwatchers/flowlogs`, `microsoft.network/applicationgatewaywebapplicationfirewallpolicies`, `microsoft.network/frontdoorwebapplicationfirewallpolicies`, `microsoft.network/applicationgateways`, `microsoft.storage/storageaccounts` | `Reader` is sufficient — no new permissions beyond AZ-1. |
