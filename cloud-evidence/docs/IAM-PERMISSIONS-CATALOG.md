...
| `inventory-assets.ts` | Inventory Workbook (`--inventory-workbook`) | Cloud Asset Inventory (RESOURCE) + OS Config inventory enrichment (INV-S3) | `roles/cloudasset.viewer` (`cloudasset.assets.list`) + `roles/osconfig.inventoryViewer` (`osconfig.inventories.list`) — the OS Config role is optional; missing it just leaves OS Name + Patch Level cells blank with a warning. |
...
| `inventory-assets.ts` | Inventory depth (`--inventory-workbook`) | Resource Graph (per-type KQL: `microsoft.compute/virtualmachines` + `microsoft.compute/disks`, `microsoft.network/networkinterfaces` + `publicipaddresses`, `microsoft.storage/storageaccounts`, `microsoft.sql/servers` + `servers/databases`, `microsoft.documentdb/databaseaccounts`, `microsoft.containerservice/managedclusters`, `microsoft.web/sites`, `microsoft.network/applicationgateways` + `loadbalancers`, `microsoft.containerregistry/registries`, `microsoft.keyvault/vaults`) | `Reader` covers all (Resource Graph projection only). |
...
| `network.ts` (`collectCnaUln`, `collectCnaRvp`, `collectSvcSnt`, `collectCnaMat`, `collectCnaRnt`) | KSI-CNA-ULN, KSI-CNA-RVP, KSI-SVC-SNT, KSI-CNA-MAT, KSI-CNA-RNT | Resource Graph — `microsoft.network/networkwatchers/flowlogs`, WAF policies (`microsoft.network/applicationgatewaywebapplicationfirewallpolicies`, `microsoft.network/frontdoorwebapplicationfirewallpolicies`), `microsoft.network/applicationgateways`, `microsoft.network/virtualnetworks`, `microsoft.network/networksecuritygroups`, `microsoft.storage/storageaccounts` | `Reader` is sufficient — no new permissions beyond AZ-1. |
| `config.ts` (`collectCnaEis`, `collectCnaIbp`, `collectCnaDfp`) | KSI-CNA-EIS, KSI-CNA-IBP, KSI-CNA-DFP | Resource Graph — `policyresources` + `authorizationresources` (`microsoft.authorization/roledefinitions`) | `Reader` is sufficient; the `authorizationresources` table for CNA-DFP needs `Microsoft.Authorization/roleDefinitions/read` which Reader provides. |
| `config.ts` (`collectSvcAcm`) | KSI-SVC-ACM | Resource Graph — `Resources` table (`microsoft.resources/deployments`) + `policyresources` (`microsoft.policyinsights/policystates` compliance ratio) | `Reader` is sufficient. |
| `config.ts` (`collectSvcEis`) | KSI-SVC-EIS | Resource Graph — `securityresources` (`microsoft.security/securescores`) | `Security Reader` is required (same constraint as `collectMlaEvc` — the SecurityResources table is gated by Defender for Cloud RBAC). |
| `supplychain.ts` (`collectCmtRmv`) | KSI-CMT-RMV | Resource Graph — `Resources` (`microsoft.containerregistry/registries` admin/anon/softDelete/trust policy) | `Reader` is sufficient. |
| `supplychain.ts` (`collectCmtVtd`) | KSI-CMT-VTD | Resource Graph — `securityresources` (`microsoft.security/securityconnectors` for DevOps env + `microsoft.security/pricings` for Containers plan) | `Security Reader` required for the SecurityResources table (same as MLA-EVC / SVC-EIS). |
| `data.ts` (`collectSvcRud`, `collectSvcVcm`, `collectSvcVri`) | KSI-SVC-RUD, KSI-SVC-VCM, KSI-SVC-VRI | Resource Graph — `Resources` table (`microsoft.storage/storageaccounts` properties: `deleteRetentionPolicy`, `isVersioningEnabled`; `microsoft.storage/storageaccounts/managementpolicies`; `microsoft.network/applicationgateways.sslProfiles`; `microsoft.apimanagement/service.hostnameConfigurations`; `microsoft.containerservice/managedclusters.serviceMeshProfile`; `microsoft.storage/storageaccounts/blobservices/containers/immutabilityPolicies`) | `Reader` is sufficient — all properties are management-plane only; no Storage Blob Data role needed (we read metadata, not blob contents). |
| `supplychain.ts` (`collectScrMon`) | KSI-SCR-MON | Resource Graph — `securityresources` (`microsoft.security/pricings` for MDVM-bearing plans + `microsoft.security/securitycontacts` for advisory routing) | `Security Reader` required for the SecurityResources table (same as MLA-EVC / SVC-EIS / CMT-VTD). |
| `inventory.ts` (`collectPiyGiv`) | KSI-PIY-GIV | Resource Graph — `Resources` (summary by type) | `Reader` is sufficient. This is the same backbone the inventory-workbook generator uses (`discover.ts`); the KSI signal is just "queries return non-zero across the configured subs". |
| `ksi-hybrids.ts` (`collectCmtRvp`, `collectInrAar`, `collectInrRpi`, `collectScrMit`, `collectSvcPrr`) | KSI-CMT-RVP, KSI-INR-AAR, KSI-INR-RPI, KSI-SCR-MIT, KSI-SVC-PRR | Resource Graph — `policyresources` (assignments + states), `Resources` table (Sentinel automationrules, Monitor activity-log alerts, Log Analytics workspace retention, ACR trust/quarantine policies, storage public-access flags), `securityresources` (Defender Containers pricing) | `Reader` covers everything except the `securityresources` pricing read used by SCR-MIT, which needs `Security Reader` (same as MLA-EVC / SCR-MON / CMT-VTD). |
| `crypto.ts` (`collectUcm`) | KSI-AFR-UCM | Resource Graph — `Resources` (`microsoft.keyvault/vaults/keys`, `microsoft.network/applicationgateways` ssl policy, `microsoft.storage/storageaccounts.encryption.requireInfrastructureEncryption`) | `Reader` is sufficient. |
| `vdr-scan.ts` (`collectVdrScan`) | KSI-AFR-VDR | Resource Graph — `securityresources` (`microsoft.security/assessments`) joined with the CISA KEV catalog | `Security Reader` required for the `securityresources` table (same as MLA-EVC / SCR-MON / CMT-VTD). The KEV join is from `docs/cisa-kev.generated.json` (committed offline); override via `CLOUD_EVIDENCE_KEV_PATH`. |
| `backup.ts` (`collectCnaOfa`) | KSI-CNA-OFA | Resource Graph — `microsoft.compute/virtualmachines` (zones), `microsoft.storage/storageaccounts` (sku.name replication) | `Reader` is sufficient. |
| `backup.ts` (`collectRplAbo`, `collectRplTrc`, `collectRplArp`, `collectRplRro`) | KSI-RPL-ABO, KSI-RPL-TRC, KSI-RPL-ARP, KSI-RPL-RRO | Resource Graph — Resources (`microsoft.recoveryservices/vaults` for ABO presence + ARP `properties.redundancySettings.standardTierStorageRedundancy`; `microsoft.dataprotection/backupvaults`) + `RecoveryServicesResources` table (`backupprotecteditems`, `backupjobs`, `backuppolicies` for RRO `properties.schedulePolicy.scheduleRunFrequency`) | `Reader` is sufficient; `RecoveryServicesResources` is exposed to Resource Graph under the same Reader RBAC as the `Resources` table. |
| `logging.ts` (`collectInrRir`) | KSI-INR-RIR | Resource Graph — `microsoft.insights/actiongroups`, `microsoft.securityinsights/automationrules` | `Reader` is sufficient. Both resource types are management-plane and exposed under the same Reader RBAC. |
| `secrets.ts` (`collectSvcAsm`) | KSI-SVC-ASM | Resource Graph — `microsoft.keyvault/vaults` (management-plane only: `properties.enableSoftDelete`, `properties.enablePurgeProtection`, `properties.enableRbacAuthorization`) | `Reader` is sufficient — we read vault metadata, **not** the data-plane (no Secrets / Keys / Certificates contents accessed). No `Key Vault Reader` data role needed. |
| `logging.ts` (`collectMlaEvc`) | KSI-MLA-EVC | Resource Graph — `securityresources` (`microsoft.security/assessments`) | `Security Reader` is required for the SecurityResources table (same as the reference-arch Defender check). |
