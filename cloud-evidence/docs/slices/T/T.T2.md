...
|---|---|---|---|---|
| 2026-06-20 | impl-t-t2 | Shipped end to end per spec (reconciled to the committed T.T1 catalogue shape). Created `core/ssdf-satisfaction-matrix.ts` (types + RFC 4122 v5 `matrix_id` + canonical serializer + 2-sheet `.xlsx` renderer) + `core/ssdf-evidence-aggregator.ts` (`buildSsdfSatisfactionMatrix` join/compute + `emitSsdfSatisfactionMatrix` sign+write) + `scripts/check-ssdf-no-silent-pass.mjs` (REO guardrail) + `tests/core/ssdf-evidence-aggregator.test.ts` (22) + `tests/core/ssdf-satisfaction-matrix.test.ts` (8). Modified `core/orchestrator.ts` (`--ssdf-attestation` flag/env + emit pass before signing), `core/submission-bundle.ts` (2 WELL_KNOWN roles), `package.json` (`check:ssdf-no-silent-pass` wired into `check:reo`). typecheck clean; 1241/1241 tests pass (+30); `npm run check:reo` (G1+G2+G3+ssdf) green. | `<TBD-step6>` | Spec §4/§5 divergence: T.T1 catalogue maps KSI (`fedramp_ksi_forward_map`) + 800-53 (`nist_800_53_r5_controls`) per PRACTICE (not per task) and has 42 active tasks (not 43; PW.3 withdrawn). Join is per-practice, attributed to each task. Q-T2-1..Q-T2-5 resolved (see notes below). Tracker process-artefact pointer + per-product tracker DB deferred (no tracker subsystem in repo) → `T.T2-17`; cosign/build-attestation state not collected → `T.T2-18`; both surfaced in `provenance.coverageDiagnostics`. KSI-envelope integrity verified against the run `manifest.json` (per-file detached signing is not the repo model). |

### Open-question resolutions (§10)

- **Q-T2-1** (unresolvable KSI in `crosswalk_ksi[]`): resolved — a forward-mapped KSI with no on-disk envelope simply contributes no pointer; absent KSIs degrade the practice toward `requires-operator-input` rather than blocking the matrix. Catalogue KSI ids are operator-curated and validated by T.T1's `validateCatalog`, so an unknown KSI cannot reach the matrix silently.
- **Q-T2-2** (cross-product evidence): resolved — by default every KSI envelope applies to every product; `ksi_to_product_map` (config.yaml `ssdf.ksi_to_product_map`) overrides per product. Test T2-18 pins the scoping.
- **Q-T2-3** (mixed tracker/KSI evidence): resolved — pointers of every kind coexist in `evidence_pointers[]`; `computeTaskStatus` treats any non-empty pointer set uniformly for the "has evidence" gate. The tracker pointer kind itself is deferred (`T.T2-17`).
- **Q-T2-4** (interim machine signature): resolved — yes; the matrix rides the same detached Ed25519 + run-manifest RFC 3161 pipeline as every other artefact. The producer-officer attestation lives on the T.T3 Common Form, never auto-signed here (REO Rule 1.10).
- **Q-T2-5** (future SP 800-218 Rev 2): resolved — T.T2 reads whatever catalogue `loadSsdfCatalog()` returns and records `catalogue_source.version` + `source_pdf_sha256` in the matrix; a Rev 2 catalogue regenerated by T.T1 flows through without T.T2 changes.
