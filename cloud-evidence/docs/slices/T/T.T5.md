...
|---|---|---|---|---|
| 2026-07-02 | impl-t-t5 | Downloaded the real NIST SP 800-218A IPD + final PDFs to `docs/sources/` (HTTP 200; sha256 `ac31ee54…` IPD / `e088c8bc…` final) with `.sha256` siblings — the catalogue MUST be extracted from real NIST text (REO Rule 1), and neither PDF was in the clean-room tree. | — | Resolved the §10/resume-checklist "confirm both PDFs exist" gate by fetching from the pinned CSRC URLs. |
| 2026-07-02 | impl-t-t5 | Built `scripts/extract-800-218A.mjs` (offline, deterministic; `pdf-parse` like T.T1) → `data/ssdf-800-218A-{ipd,final}.json` + `docs/sources/ssdf-800-218A-delta.json`. Final: 20 practices, 48 tasks, 86 R/C/N items, 6 new AI tasks. | `<TBD-step6>` | **Spec reconciliation (T.T5-16):** real 800-218A uses `<task>.R/.C/.N<n>` items, NOT the spec's assumed `<task>.A<n>`. **(T.T5-17):** 800-218A re-adds PW.3.*/PS.1.2/1.3/PO.5.3 absent from base v1.1 → `base_task_present:false`. Added `build:ssdf-ai-catalog` npm script + lint-no-stubs allowlist entry. |
| 2026-07-02 | impl-t-t5 | Implemented `core/ssdf-ai-extension.ts` (catalogue loader + integrity, model-card walker w/ graceful absence, pure derivation engine per §6.6 adapted for base-absent tasks, evidence-pointer merge, rollup, guardrail-safe augmented-matrix re-emit, provenance + detached Ed25519 sign, additive coverage) + `core/ssdf-ai-extension-xlsx.ts` (Summary + per-product A..O + IPD-vs-final delta + statutory-lineage worksheets). | `<TBD-step6>` | Augmentations interleave under each base task as `ai_augmentations[]` (not sibling rows) so `check:ssdf-no-silent-pass` still sees only base tasks; new AI tasks appended as `requires-operator-input` rows when evidence-less. |
| 2026-07-02 | impl-t-t5 | Wired orchestrator (new step under `--ssdf-attestation` after T.T2, before T.T3), 3 `submission-bundle` WELL_KNOWN roles, `config.yaml#ssdf.{ai_augmentation_enabled,primary_catalogue,ai_products_in_scope}`, and a minimal T.T3 reader diff (`ssdf-common-form.ts` skips `.augmented.json` to avoid double-counting). | `<TBD-step6>` | **(T.T5-20):** LOOP-O.O5 unshipped → the step no-ops (`coverage:skipped`, reason `no-model-cards`) — realizable-core / graceful-degradation posture, same as T.T2/T.T3/T.T4/W.W3/W.W4. No fabricated AI evidence (REO Rule 4). |
| 2026-07-02 | impl-t-t5 | Shipped 33 tests (`tests/core/ssdf-ai-extension.test.ts` 29 + `…-xlsx.test.ts` 4) covering §8 T5-1..25 (reconciled: R/C/N ids; RFC 3161 via run-manifest TSR not per-file `.tsr`, T.T5-21). typecheck 0, 1341/1341 tests, `check:reo` (G1/G2/G3 + no-silent-pass) all green. | `<TBD-step6>` | Deferred: tracker page `/ssdf/ai-augmentation` (T.T5-22 — no tracker subsystem). Shipping T.T5 completes LOOP-T (5 of 5). |

**Open-questions resolution (per §10):** Q1 (per-augmentation confidence_score) — deferred to a future T.T6 as recommended; status remains the derivation-table value. Q2 (POA&M routing) — kept the A.A1 route (T.T5 surfaces `not-satisfied`/`requires-operator-input` rows; A.A1 emits the POA&M). Q3 (record secondary catalogue) — implemented: the result carries `catalogue_version` + `catalogue_secondary_version`. Q4 (statutory-lineage FAQ worksheet) — implemented as the "Statutory lineage" worksheet + the `eo_lineage[]` field. Q5 (LOOP-S.S1 cross-ref) — no T.T5-side change; S.S1 reads `ssdf-satisfaction-matrix.augmented.json` when present (documented in the bundle role description).
