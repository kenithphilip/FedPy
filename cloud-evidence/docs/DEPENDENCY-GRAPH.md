# Dependency Graph — every core slice across LOOP-A through LOOP-S + LOOP-T + LOOP-W + LOOP-X (+ CIRCIA + SEC 8-K overlays)

> **Scope:** This graph covers FedPy CORE only. LOOP-U (Privacy),
> LOOP-V (HIPAA), LOOP-Y (CJIS/IRS), LOOP-Z (ISO international) have
> been scope-fenced to `docs/roadmap/` and are NOT part of this graph.
> See `cloud-evidence/CLAUDE.md` "Scope Guard" block and
> `docs/roadmap/README.md` for the rationale.
...
graph TD
  ...
  %% =========================================================
  %% LOOP-X — Zero Trust Architecture (CORE, in-scope for FedRAMP-serving CSPs)
  %% OMB M-22-09 + NIST SP 800-207/207A + CISA ZTMM v2.0
  %% =========================================================
  A.A5 --> X.X1[X.X1 ZT pillar inventory]
  X.X1 --> X.X2[X.X2 NIST SP 800-207 PDP/PEP placement]
  INVCHAIN --> X.X2
  X.X2 --> X.X3[X.X3 NIST SP 800-207A cloud-native ZTA]
  E2SBOM --> X.X3
  J.J3 --> X.X3
  X.X1 --> X.X4[X.X4 CISA ZTMM v2.0 maturity scorecard]
  X.X2 --> X.X4
  X.X3 --> X.X4
  A.A4 --> X.X4
  A.A5 --> X.X4
  X.X2 --> X.X5[X.X5 PDP/PEP integration evidence]
  X.X3 --> X.X5

  %% =========================================================
  %% LOOP-U, LOOP-V, LOOP-Y, LOOP-Z — OUT OF CORE SCOPE
  %% Moved to docs/roadmap/. Dependency edges removed from this
  %% graph because they are not part of the FedRAMP authorization
  %% pipeline. See docs/roadmap/README.md and CLAUDE.md Scope Guard.
  %% =========================================================
...
| **X.X4** | X.X1, X.X2, X.X3, A.A4, A.A5 | (terminal — CISA ZTMM v2.0 scorecard emitter) |
| **X.X5** | X.X2, X.X3, INV-S | (terminal — PDP/PEP integration telemetry) |
| **U.U***, **V.V***, **Y.Y***, **Z.Z*** | (out of core scope) | See `docs/roadmap/` — moved out of FedPy core; not part of FedRAMP authorization pipeline. |
