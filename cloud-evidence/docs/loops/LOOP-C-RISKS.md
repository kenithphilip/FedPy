...
### C.C2 — ISCP + Test AAR
- **C-C2-1**: RPL evidence shape drift across collector versions. — MITIGATED (2026-07-07, `HASHTBD`): the narrow `KsiEvidence` interface (`iscp-emit.ts`) reads only `ksi_id`/`rollup.pass`/`collected_at`; `readKsiEvidence` is JSON-parse-safe (never throws, skips malformed/missing files), so new backup-feature fields don't break the emitter.
