...
| B.B3-9 | med | AO approval lacks second-factor | Out of scope for B.B3 ship; existing session + role + signature sufficient; file follow-up | open |
| B.B3-10 | low | Orphaned acceptance (finding remediated separately) | Out of scope; UI surfaces stale review item; follow-up task | open |
...
| B.B4-9 | med | H.4 attachment SHA-256 signed at CC create; if uploaded after, signature invalidates | UI flow forces upload BEFORE submit | open |
