...
| B.B5-11 | low | **No SheetJS in either workspace; the spec's XLSX round-trip test cannot use it** [discovered impl-b-b5, 2026-07-02]. Per-slice §8 test 13 assumed `xlsx` was a dev dep. | Realised as a valid-OOXML round-trip: the `.xlsx` is a dependency-free store-only OOXML zip; tests parse the store-only local-file headers directly (same approach as `inventory-workbook.test.ts` / `ssdf-ai-extension-xlsx.test.ts`) and assert the 20-column header, frozen pane, conditional-fill styles, and wrapped Description. Any consumer (Excel/SheetJS) still round-trips the output; the test just doesn't depend on SheetJS. | mitigated |
| B.B5-12 | low | **XLSX renderer duplicated across workspaces** [discovered impl-b-b5, 2026-07-02]. The tracker is a separate npm workspace and cannot import `cloud-evidence/core/risk-register-xlsx.ts`, so `tracker/server/risk-register-xlsx.ts` is a self-contained copy (same 20-column layout + frozen pane + conditional formatting + a minimal store-only zip writer). | A future column/style change MUST be applied to BOTH files. The two share identical column order + style indices; drift would surface as differing XLSX structure. Tracked alongside B.B4-12 (NIST-catalog copy drift) as a cross-workspace-sync risk. | open |
| B.B5-13 | med | **Tracker aggregated endpoint returns a subset of the RA-3 register** [discovered impl-b-b5, 2026-07-02]. `GET /api/risk-register` joins only tracker-resident data (organisational risks + approved acceptances); finding-sourced entries live in the collector's `out/poam.json`, which is not tracker-resident. | The authoritative, finding-inclusive RA-3 register is the collector's signed `out/risk-register.json` (emitted by `core/risk-register.ts` via `--risk-register`). The tracker endpoint + `/export.xlsx` are the UI-facing subset for operator visibility; acceptance entries there carry `REQUIRES-OPERATOR-INPUT` bands (the tracker cannot derive them without poam.json). A future slice could ingest a `risk-scores.json`/`poam.json` upload into the tracker to unify the two (spec §8 build-step 3 / Q2). | open |
