...
| W.W4-EXT-1 | high | **Discovered 2026-06-18 during W.W4 impl.** The spec §5.3/§7 assumes a tracker subsystem — a Postgres `section889_annual_reps` table, REST routes (`tracker/server/routes/section889.ts` — `GET /annual-reps`, `GET /annual-reps/:id`, `POST /annual-reps/:id/sam-receipt`), a React review/sign-off + SAM-receipt paste-back UI (`tracker/ui/section889-annual-rep-status.tsx`, `nav.tsx`), and the T-60/T-30/T-7 expiry-nag daemon — none of which exist in this repo (no `pg`/`express`/`react` deps; every prior slice ships as `core/*.ts` + `tests/core/*.test.ts`). W.W4 shipped the **full regulatory deliverable** (signed canonical-JSON envelope + `.docx` representation pair, screen-driven (c)(1)/(c)(2) answers, linked W.W3 incidents, Marketplace badge feed, submission-bundle registration). The tracker-resident operator-workflow layer (DB rows, sign-off UI, SAM-receipt paste-back, expiry nags, `.ics` renewal reminder) is **DEFERRED** until the shared tracker subsystem is built. | Interim: the append-only ledger `section889-annual-reps.jsonl` is the durable delta + continuity + flip-detection substrate; the `marketplace-section889-badge.json` feed surfaces validity/expiry to LOOP-Q.Q1. Owner: tracker-subsystem buildout (forward-spec shared dependency, same posture as W.W2-EXT-1 + W.W3-17). | open (follow-up) |
| W.W4-EXT-2 | low | **Discovered 2026-06-18.** Spec §4.4/§11 (T10) want an officer keyring with per-key `expiresAt` validation. No keyring abstraction exists — `core/sign.ts` signs with `EVIDENCE_SIGNING_KEY_PATH` (or an ephemeral dev key). W.W4 validates `authorized_officer.signing_key_id` as a required non-empty operator label (recorded verbatim in the envelope) and records the actual signature fingerprint in `provenance.signingKeyId`; the keyring-expiry check (T10) is deferred with the tracker subsystem. | When the tracker keyring lands, gate emission on `keyring.get(id).expiresAt > signed_at`; until then the operator manages key validity out-of-band per the runbook. | open (follow-up, with tracker) |
| W.W4-EXT-3 | low | **Discovered 2026-06-18.** Input-shape divergence: spec §4.1 names the W.W2 input `out/prohibited-vendors-matches.json` (`ProhibitedVendorMatches`); the shipped W.W2 emits `out/prohibited-vendors-screen-result.json` (`ProhibitedVendorScreenResult`). W.W4 consumes the real artifact (signature-verified) — same reconciliation W.W3 made. A reader following the stale spec name would not find the file. | Documented in W.W4.md §12; the orchestrator + `emitSection889AnnualRep` default to `SCREEN_RESULT_FILENAME`; spec §4.1 is the snapshot, the per-slice §12 log is the live record. | resolved (by design) |
| W.W4-EXT-4 | low | **Discovered 2026-06-18.** RFC 3161 + OOXML divergences: (a) the envelope records `rfc3161_timestamp.status:'pending'` rather than a per-artifact TST (manifest-level TST attaches at run signing via `core/timestamp.ts` — same as W.W2/W.W3; no per-artifact `timestampDigest` primitive exists, so spec §6.12/T20's mocked-TSA-digest path is not applicable); (b) the `.docx` emits the 5 minimal-conformant OOXML parts the proven `core/zip.ts` byte-reproducible pattern produces, not the 8 parts spec §5.2/T15 enumerate. | The detached Ed25519 signature + `.sig` sidecar is the integrity mechanism; T13/T14 lock canonical + `.docx` byte-stability; T15 asserts the actual declared parts. Attach a per-envelope TST when a TSA is configured. | open (follow-up) |
