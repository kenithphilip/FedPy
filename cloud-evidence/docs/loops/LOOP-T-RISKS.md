...
| T.T5-16 | med | **Augmentation-identifier scheme divergence (discovered during impl, 2026-07-02).** The T.T5 spec §2.6/§4.1 + several planning risks (T.T5-6 "PW.A.1") assumed 800-218A augmentation ids of the form `<task>.A<n>`. The published 800-218A (both IPD + final) uses per-task Recommendation / Consideration / Note items with ids `<task>.R<n>` / `.C<n>` / `.N<n>` (final §3 legend: "'PO.1.2.R1' is the first recommendation for task PO.1.2"). | RESOLVED — `scripts/extract-800-218A.mjs` parses the real R/C/N scheme; the catalogue + `core/ssdf-ai-extension.ts` carry `item_type: 'R'|'C'|'N'` and real ids. No `.A<n>` ids are invented (REO Rule 1). | resolved |
| T.T5-17 | med | **Base-absent AI tasks (discovered 2026-07-02).** 800-218A re-introduces the PW.3 practice + tasks PW.3.1/3.2/3.3, PS.1.2, PS.1.3, PO.5.3 that base SSDF v1.1 (42 tasks) does NOT carry, so they have no T.T2 matrix parent to inherit. The spec §6.2 "assert every parent_task_id exists in base … throw" is therefore too strict. | RESOLVED — the catalogue tags these `base_task_present:false` + `not-part-of-ssdf-1.1`; the aggregator treats a new AI task with no AI-specific evidence as `requires-operator-input` (never a silent pass, REO Rule 4); the loader only throws when a task *flagged* base-present is actually absent from base. | resolved |
| T.T5-18 | low | **PDF page-break column interleaving (discovered 2026-07-02).** A Table-1 row that spans a page boundary interleaves a reference-column chunk between its recommendation items. The extractor recovers split R/C/N items (an item marker re-enters rec mode), but a page-break-split rec *continuation* can land in that task's `informative_references`. Bounded to a handful of page-break tasks; the text is verbatim NIST text, mis-columned. | Machine-consumed fields (item statements, ids, status derivation) are unaffected — derivation keys off parent status + AI-evidence presence, not refs. Documented; a future extractor pass can add cross-page cell reassembly. | open |
| T.T5-19 | low | **IPD (withdrawn draft) as primary catalogue.** Spec §4.6 defaults `primary_catalogue: 'IPD'` "per user task brief", but the IPD was obsoleted by the final publication (2024-07-26) and the extractor parses the final more cleanly. | Both catalogues are committed + extracted verbatim; the emitted `docs/sources/ssdf-800-218A-delta.json` + the operator-configurable drift threshold surface any divergence; operators may set `primary_catalogue: final`. The default honours the spec. | open |
| T.T5-20 | med | **LOOP-O.O5 model-card registry unimplemented.** T.T5's AI scope gate is `out/model-cards/*.json` (LOOP-O.O5), which does not exist in this repo. | RESOLVED (realizable-core posture, per STATUS "Next priority" directive; same as T.T2/T.T3/T.T4/W.W3/W.W4) — the orchestrator step is fully wired + tested against fixtures but no-ops (`coverage:skipped`, reason `no-model-cards`) until LOOP-O.O5 ships model cards. No fabricated AI evidence. | resolved |
| T.T5-21 | low | **RFC 3161 coverage via run manifest, not per-file `.tsr`.** Spec §5.4/§8 T22 imply a per-file `.tsr`. T.T5 follows the T.T2/T.T3/T.T4 pattern: detached Ed25519 per artefact + RFC 3161 coverage from the run-level manifest TSR. | RESOLVED — the sign test asserts `.sig` + signature verification; the run manifest (`core/sign.ts` `signRun`) provides the timestamp. Consistent with the rest of LOOP-T. | resolved |
| T.T5-22 | low | **Deferred tracker page `/ssdf/ai-augmentation`.** SPEC §5 Table row 10 lists a React worksheet page; no tracker subsystem exists in this repo (no `pg`/`express`/`react`/`better-sqlite3`), the same posture as T.T2/T.T3/T.T4/W.W3/W.W4. | Deferred — the signed `out/ssdf-ai-augmentation.json` + `.xlsx` operator workbook are the realizable-core deliverables; the tracker pane binds them when the tracker layer ships. | deferred |
