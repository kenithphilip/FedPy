...
export async function collectScrMit(ctx: CollectorContext): Promise<ProviderBlock> {
  ...
  findings.push(finding({
    ...
    gap: { description: 'No supply-chain mitigation primitive observed in Azure — image deployments are not gated by signing/quarantine or scanned by Defender.', affected_resources: registries.rows.filter((r: any) => !mitigated.includes(r)).slice(0, 50).map((r: any) => ({ type: 'azure_container_registry', identifier: r.id, attributes: { name: r.name, trust: r.trust, quarantine: r.quarantine } })) },
    remediation: {
