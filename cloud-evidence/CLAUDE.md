...
3a. **`docs/PER-SLICE-RESUME-PROMPT.md`** — the canonical session-bootstrap prompt. Use this when opening a fresh Claude Code session to ship ANY slice. §1 is the universal template (parameterize `<SLICE-ID>` / `<LOOP>` / `<SLICE-TITLE>` / `<DEPENDENCY-LOOPS>`); §2 is the populated instance for W.W1 (the next-priority slice). The prompt enumerates every file to read, every REO rule, every primitive to reuse, every file to update at completion, and the file-by-file completion checklist — so a fresh session has zero ambiguity and zero context loss.
